...
public class NoneApsImpl implements ApsInterface {
    ...
    public int addFrameBuff(ApsCaptureParam apsCaptureParam) {
        ...
        captureFrame.mData = Util.convertNV21DataToJpeg(Util.getYuvDataWithoutPadding(image, 17, null), image.getWidth(), image.getHeight());
        synchronized (this.mCaptureQueueLock) {
    ...
    public int addPreviewFrameBuff(ApsPreviewParam apsPreviewParam, ApsWatermarkParam apsWatermarkParam) {
        ...
        if (TextUtils.isEmpty(str)) {
            CameraUnitLog.w(TAG, "addPreviewFrameBuff, camera id is null");
        } else {
            previewFrame.mCameraId = CameraCharacteristicsHelper.isFrontCamera(Integer.parseInt(str)) ? 1 : 0;
        }
