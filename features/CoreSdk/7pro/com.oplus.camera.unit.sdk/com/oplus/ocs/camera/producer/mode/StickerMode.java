...
public class StickerMode extends VideoMode {
    ...
    protected long getHalMemory() {
        return ((Integer) CameraAdapterUtils.getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_STICKER, 0)).intValue();
    }
    ...
    public boolean needAddToTarget(String str, String str2, @NonNull SurfaceKey surfaceKey, PreviewParameter.Builder builder) {
        if (Parameter.ParameterStage.BEFORE_TAKE_PICTURE.equals(str2) && CameraSessionEntity.SurfaceUsage.PREVIEW.equals(surfaceKey.getUsage())) {
            return true;
        }
    ...
    public void updateFpsRange(Parameter parameter, PreviewParameter.Builder builder, String str) {
        Range[] rangeArr = (Range[]) CameraCharacteristicsHelper.getCameraCharacteristicsWrapper(str).get().get(CameraCharacteristics.CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES);
        ...
        for (int i3 = 0; i3 < rangeArr.length; i3++) {
            if (i2 > ((Integer) rangeArr[i3].getLower()).intValue() && 30 == ((Integer) rangeArr[i3].getUpper()).intValue()) {
                i2 = ((Integer) rangeArr[i3].getLower()).intValue();
                i = i3;
            }
        }
        if (i >= 0) {
            CameraUnitLog.d(TAG, "updateFpsRange, set CONTROL_AE_TARGET_FPS_RANGE fps range to: " + rangeArr[i]);
            builder.set((CaptureRequest.Key<CaptureRequest.Key>) CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE, (CaptureRequest.Key) rangeArr[i]);
            return;
        }
        super.updateFpsRange(parameter, builder, str);
    }
