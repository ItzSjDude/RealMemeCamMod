...
public class OPlusRefreshRateManager implements IOPlusRefreshRateManager {
    ...
    @Override // com.oplus.vrr.IOPlusRefreshRateManager
    public void screenStateChangeWarning(int state) {
        IOPlusRefreshRate service = getDefault();
        if (service != null) {
            try {
                service.screenStateChangeWarning(state);
            } catch (RemoteException e) {
            }
        } else {
            Slog.w(TAG, "OPlusRefreshRate disconnected");
    ...
    @Override // com.oplus.vrr.IOPlusRefreshRateManager
    public boolean setVsyncConfig(int status, float appOffset, float sfOffset, String identity) {
        IOPlusRefreshRate service = getDefault();
        if (service != null) {
            try {
                boolean result = service.setVsyncConfig(status, appOffset, sfOffset, identity);
                return result;
            } catch (RemoteException e) {
                return false;
            }
