...
public class LatencyTracker {
    ...
    private LatencyTracker() {
        BackgroundThread.getHandler().post(new Runnable() { // from class: com.android.internal.util.LatencyTracker$$ExternalSyntheticLambda1
            ...
            public final void run() {
                this.f$0.m7115lambda$new$0$comandroidinternalutilLatencyTracker();
            }
        });
    ...
    /* synthetic */ void m7115lambda$new$0$comandroidinternalutilLatencyTracker() {
        ...
        synchronized (this.mLock) {
            if (isEnabled()) {
                ...
                session.begin(new Runnable() { // from class: com.android.internal.util.LatencyTracker$$ExternalSyntheticLambda0
                    @Override // java.lang.Runnable
                    public final void run() {
                        this.f$0.m7116lambda$onActionStart$1$comandroidinternalutilLatencyTracker(action);
                    }
                });
    ...
    public void m7116lambda$onActionStart$1$comandroidinternalutilLatencyTracker(int action) {
        synchronized (this.mLock) {
            Session session = this.mSessions.get(action);
            if (session == null) {
                return;
    ...
    public void logAction(int action, int duration) throws IOException {
    ...
    static class Session {
        ...
        Session(int action, String tag) {
            ...
            this.mTag = tag;
            if (TextUtils.isEmpty(tag)) {
                nameOfAction = LatencyTracker.getNameOfAction(LatencyTracker.STATSD_ACTION[action]);
            } else {
                nameOfAction = LatencyTracker.getNameOfAction(LatencyTracker.STATSD_ACTION[action]) + "::" + tag;
            }
