...
public class NativeLibraryHelper {
    ...
    public static void createNativeLibrarySubdir(File path) throws IOException, ErrnoException {
        if (!path.isDirectory()) {
            path.delete();
            if (!path.mkdir()) {
    ...
    public static int copyNativeBinariesForSupportedAbi(Handle handle, File libraryRoot, String[] abiList, boolean useIsaSubdir, boolean isIncremental) throws IOException, ErrnoException {
        File subDir;
        int abi = findSupportedAbi(handle, abiList);
        if (abi < 0) {
            return abi;
    ...
    public static int copyNativeBinariesWithOverride(Handle handle, File libraryRoot, String abiOverride, boolean isIncremental) throws ErrnoException {
        int copyRet;
        int copyRet2;
        try {
            if (handle.multiArch) {
