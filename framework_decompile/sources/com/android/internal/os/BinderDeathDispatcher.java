...
public class BinderDeathDispatcher<T extends IInterface> {
    ...
    public int linkToDeath(T target, IBinder.DeathRecipient recipient) {
        ...
        synchronized (this.mLock) {
            BinderDeathDispatcher<T>.RecipientsInfo recipientsInfo = this.mTargets.get(ib);
            if (recipientsInfo == null) {
                recipientsInfo = new RecipientsInfo(ib);
                try {
                    ib.linkToDeath(recipientsInfo, 0);
                    this.mTargets.put(ib, recipientsInfo);
                } catch (RemoteException e) {
                    return -1;
                }
            }
            recipientsInfo.mRecipients.add(recipient);
