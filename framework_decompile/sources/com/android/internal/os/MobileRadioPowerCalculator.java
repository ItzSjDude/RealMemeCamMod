    ...
    public MobileRadioPowerCalculator(PowerProfile profile) {
        ...
        if (profile.getAveragePowerOrDefault(PowerProfile.POWER_RADIO_ON, -1.0d) != -1.0d) {
            for (int i2 = 0; i2 < NUM_SIGNAL_STRENGTH_LEVELS; i2++) {
                this.mIdlePowerEstimators[i2] = new UsageBasedPowerEstimator(profile.getAveragePower(PowerProfile.POWER_RADIO_ON, i2));
            }
        } else {
            ...
            for (int i3 = 1; i3 < NUM_SIGNAL_STRENGTH_LEVELS; i3++) {
                this.mIdlePowerEstimators[i3] = new UsageBasedPowerEstimator(Math.max(1.0d, idle / 256.0d));
            }
        }
