...
public final class BigIntegers {
    ...
    public static void asUnsignedByteArray(BigInteger value, byte[] buf, int off, int len) {
        ...
        int start = (bytes[0] != 0 || bytes.length == 1) ? 0 : 1;
        int count = bytes.length - start;
        if (count > len) {
            throw new IllegalArgumentException("standard length exceeded for value");
        }
    ...
    public static BigInteger createRandomPrime(int bitLength, int certainty, SecureRandom random) throws IllegalArgumentException {
        BigInteger rv;
        if (bitLength < 2) {
