...
public class ECAlgorithms {
    ...
    private static ECPoint implSumOfMultiplies(boolean[] zArr, WNafPreCompInfo[] wNafPreCompInfoArr, byte[][] bArr) {
        ...
        while (i2 >= 0) {
            ...
            while (i3 < length) {
                byte[] bArr3 = bArr[i3];
                ?? r12 = i2 < bArr3.length ? bArr3[i2] : z;
                if (r12 != 0) {
                    int iAbs = Math.abs((int) r12);
                    WNafPreCompInfo wNafPreCompInfo = wNafPreCompInfoArr[i3];
                    eCPointAdd = eCPointAdd.add(((r12 < 0 ? true : z) == zArr[i3] ? wNafPreCompInfo.getPreComp() : wNafPreCompInfo.getPreCompNeg())[iAbs >>> 1]);
                }
                i3++;
                z = false;
            }
            if (eCPointAdd == infinity) {
                ...
                if (i > 0) {
                    eCPointTwicePlus = eCPointTwicePlus.timesPow2(i);
                    i = 0;
                }
        ...
        if (i > 0) {
            return eCPointTwicePlus.timesPow2(i);
        }
