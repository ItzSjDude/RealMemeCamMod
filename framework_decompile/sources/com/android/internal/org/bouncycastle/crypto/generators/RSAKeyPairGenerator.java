...
public class RSAKeyPairGenerator implements AsymmetricCipherKeyPairGenerator {
    ...
    protected BigInteger chooseRandomPrime(int bitlength, BigInteger e, BigInteger sqrdBound) throws IllegalArgumentException {
        for (int i = 0; i != bitlength * 5; i++) {
            BigInteger p = BigIntegers.createRandomPrime(bitlength, 1, this.param.getRandom());
            BigInteger bigIntegerMod = p.mod(e);
            BigInteger bigInteger = ONE;
            if (!bigIntegerMod.equals(bigInteger) && p.multiply(p).compareTo(sqrdBound) >= 0 && isProbablePrime(p) && e.gcd(p.subtract(bigInteger)).equals(bigInteger)) {
                return p;
            }
        }
