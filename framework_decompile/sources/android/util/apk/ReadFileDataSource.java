...
class ReadFileDataSource implements DataSource {
    ...
    @Override // android.util.apk.DataSource
    public void feedIntoDataDigester(DataDigester md, long offset, int size) throws DigestException, IOException, ErrnoException {
        try {
            byte[] buffer = new byte[Math.min(size, 1048576)];
            long start = this.mFilePosition + offset;
            long end = start + size;
            long jMin = Math.min(size, 1048576);
            long pos = start;
            while (true) {
                long curSize = jMin;
                if (pos < end) {
                    ...
                    try {
                        ...
                        pos += readSize;
                        jMin = Math.min(end - pos, 1048576L);
