...
public class ApkSignatureSchemeV3Verifier {
    ...
    private static VerifiedSigner verify(String apkFile, boolean verifyIntegrity) throws SignatureNotFoundException, IOException, SecurityException {
        RandomAccessFile apk = new RandomAccessFile(apkFile, "r");
        try {
            VerifiedSigner verifiedSignerVerify = verify(apk, verifyIntegrity);
            apk.close();
    ...
    private static VerifiedSigner verify(RandomAccessFile apk, boolean verifyIntegrity) throws SignatureNotFoundException, IOException, SecurityException {
    ...
    public static SignatureInfo findSignature(RandomAccessFile apk) throws SignatureNotFoundException, IOException {
        return findSignature(apk, APK_SIGNATURE_SCHEME_V3_BLOCK_ID);
    }

    private static SignatureInfo findSignature(RandomAccessFile apk, int blockId) throws SignatureNotFoundException, IOException {
    ...
    private VerifiedSigner verify(SignatureInfo signatureInfo, int blockId) throws InvalidKeySpecException, NoSuchAlgorithmException, SignatureException, IOException, InvalidKeyException, SecurityException, CertificateException, PlatformNotSupportedException, InvalidAlgorithmParameterException {
        byte[] verityRootHash;
    ...
    private Pair<X509Certificate[], ApkSigningBlockUtils.VerifiedProofOfRotation> verifySigner(ByteBuffer signerBlock, Map<Integer, byte[]> contentDigests, CertificateFactory certFactory) throws InvalidKeySpecException, NoSuchAlgorithmException, SignatureException, IOException, InvalidKeyException, SecurityException, PlatformNotSupportedException, InvalidAlgorithmParameterException {
    ...
    private Pair<X509Certificate[], ApkSigningBlockUtils.VerifiedProofOfRotation> verifyAdditionalAttributes(ByteBuffer attrs, List<X509Certificate> certs, CertificateFactory certFactory) throws NoSuchAlgorithmException, SignatureException, IOException, InvalidKeyException, SecurityException, PlatformNotSupportedException, InvalidAlgorithmParameterException {
    ...
    static byte[] getVerityRootHash(String apkPath) throws SignatureNotFoundException, IOException, SecurityException {
    ...
    static byte[] generateApkVerity(String apkPath, ByteBufferFactory bufferFactory) throws SignatureNotFoundException, NoSuchAlgorithmException, DigestException, IOException, SecurityException {
        RandomAccessFile apk = new RandomAccessFile(apkPath, "r");
        try {
            SignatureInfo signatureInfo = findSignature(apk);
            byte[] bArrGenerateApkVerity = VerityBuilder.generateApkVerity(apkPath, bufferFactory, signatureInfo);
            apk.close();
