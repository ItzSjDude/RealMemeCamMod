...
public abstract class VerityBuilder {
    ...
    public static VerityResult generateApkVerityTree(RandomAccessFile apk, SignatureInfo signatureInfo, ByteBufferFactory bufferFactory) throws NoSuchAlgorithmException, DigestException, IOException, SecurityException {
        return generateVerityTreeInternal(apk, bufferFactory, signatureInfo);
    }

    private static VerityResult generateVerityTreeInternal(RandomAccessFile apk, ByteBufferFactory bufferFactory, SignatureInfo signatureInfo) throws NoSuchAlgorithmException, DigestException, IOException, SecurityException {
    ...
    public static byte[] generateFsVerityRootHash(String apkPath, byte[] salt, ByteBufferFactory bufferFactory) throws NoSuchAlgorithmException, DigestException, IOException {
        ...
        try {
            ...
            output.order(ByteOrder.LITTLE_ENDIAN);
            ByteBuffer tree = slice(output, 0, merkleTreeSize);
            byte[] bArrGenerateFsVerityTreeInternal = generateFsVerityTreeInternal(apk, salt, levelOffset, tree);
            apk.close();
    ...
    static byte[] generateApkVerity(String apkPath, ByteBufferFactory bufferFactory, SignatureInfo signatureInfo) throws SignatureNotFoundException, NoSuchAlgorithmException, DigestException, IOException, SecurityException {
        RandomAccessFile apk = new RandomAccessFile(apkPath, "r");
        try {
    ...
    private static void consumeByChunk(DataDigester digester, DataSource source, int chunkSize) throws DigestException, IOException {
    ...
    private static void generateFsVerityDigestAtLeafLevel(RandomAccessFile file, byte[] salt, ByteBuffer output) throws NoSuchAlgorithmException, DigestException, IOException {
    ...
    private static void generateApkVerityDigestAtLeafLevel(RandomAccessFile apk, SignatureInfo signatureInfo, byte[] salt, ByteBuffer output) throws NoSuchAlgorithmException, DigestException, IOException {
        BufferedDigester digester = new BufferedDigester(salt, output);
    ...
    private static byte[] generateFsVerityTreeInternal(RandomAccessFile apk, byte[] salt, int[] levelOffset, ByteBuffer output) throws NoSuchAlgorithmException, DigestException, IOException {
    ...
    private static byte[] generateVerityTreeInternal(RandomAccessFile apk, SignatureInfo signatureInfo, byte[] salt, int[] levelOffset, ByteBuffer output) throws NoSuchAlgorithmException, DigestException, IOException {
