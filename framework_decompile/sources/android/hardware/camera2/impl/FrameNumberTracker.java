...
public class FrameNumberTracker {
    ...
    public void close() {
        for (LinkedList linkedList : this.mPendingFrameNumbersWithOtherType) {
            if (linkedList != null) {
                linkedList.clear();
            }
        }
        for (LinkedList linkedList2 : this.mPendingFrameNumbers) {
            if (linkedList2 != null) {
                linkedList2.clear();
            }
        }
        HashMap<Long, List<CaptureResult>> map = this.mPartialResults;
        if (map != null) {
            map.clear();
        }
    }
