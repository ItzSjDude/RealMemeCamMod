...
public class AndroidKeyStoreKeyFactorySpi extends KeyFactorySpi {
    ...
    protected <T extends KeySpec> T engineGetKeySpec(Key key, Class<T> keySpecClass) throws InvalidKeySpecException {
        ...
        boolean zEquals = RSAPublicKeySpec.class.equals(keySpecClass);
        ...
        if (zEquals) {
            ...
            StringBuilder sbAppend = new StringBuilder().append("Obtaining RSAPublicKeySpec not supported for ").append(key.getAlgorithm()).append(" ");
            if (!(key instanceof AndroidKeyStorePrivateKey)) {
                str = "public";
            }
            throw new InvalidKeySpecException(sbAppend.append(str).append(" key").toString());
        }
        if (ECPublicKeySpec.class.equals(keySpecClass)) {
            ...
            StringBuilder sbAppend2 = new StringBuilder().append("Obtaining ECPublicKeySpec not supported for ").append(key.getAlgorithm()).append(" ");
            if (!(key instanceof AndroidKeyStorePrivateKey)) {
                str = "public";
            }
            throw new InvalidKeySpecException(sbAppend2.append(str).append(" key").toString());
        }
