...
public class NetworkSecurityTrustManager extends X509ExtendedTrustManager {
    ...
    public NetworkSecurityTrustManager(NetworkSecurityConfig config) throws NoSuchAlgorithmException, IOException, KeyStoreException, CertificateException {
        if (config == null) {
    ...
    public List<X509Certificate> checkServerTrusted(X509Certificate[] certs, String authType, String host) throws NoSuchAlgorithmException, CertificateException {
        List<X509Certificate> trustedChain = this.mDelegate.checkServerTrusted(certs, authType, host);
        checkPins(trustedChain);
        return trustedChain;
    }

    private void checkPins(List<X509Certificate> chain) throws NoSuchAlgorithmException, CertificateException {
