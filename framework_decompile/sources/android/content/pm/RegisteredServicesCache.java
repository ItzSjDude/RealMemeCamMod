...
public abstract class RegisteredServicesCache<V> {
    ...
    private IRegisteredServicesCacheExt registeredServicesCacheExt;
    ...
    public RegisteredServicesCache(Context context, String interfaceName, String metaDataName, String attributeName, XmlSerializerAndParser<V> serializerAndParser) throws IOException {
        Object obj = new Object();
        this.mServicesLock = obj;
        ...
        this.registeredServicesCacheExt = RegisteredServicesCacheExtPlugin.constructor.newInstance(obj);
    }
    ...
    public void handlePackageEvent(Intent intent, int userId) {
        ...
        if (!isRemoval || !replacing) {
            ...
            if (Intent.ACTION_EXTERNAL_APPLICATIONS_AVAILABLE.equals(action) || Intent.ACTION_EXTERNAL_APPLICATIONS_UNAVAILABLE.equals(action)) {
                ...
                if (Intent.ACTION_PACKAGE_CHANGED.equals(action) && (intent.getFlags() & 1073741824) != 0) {
                    generateServicesMap(uids, userId, true);
                    return;
                }
            }
    ...
    private void notifyListener(final V type, final int userId, final boolean removed) {
        ...
        handler.post(new Runnable() { // from class: android.content.pm.RegisteredServicesCache$$ExternalSyntheticLambda0
            ...
            public final void run() {
                RegisteredServicesCache.lambda$notifyListener$0(listener, type, userId, removed);
            }
        });
    }
    ...
    public void updateServices(int userId) throws PackageManager.NameNotFoundException {
        synchronized (this.mServicesLock) {
            UserServices<V> user = findOrCreateUserLocked(userId);
            if (user.services == null) {
                return;
    ...
    private void generateServicesMap(int[] changedUids, int userId) {
        generateServicesMap(changedUids, userId, false);
    }

    private void generateServicesMap(int[] changedUids, final int userId, boolean forComponentChange) {
        boolean changed;
        ArrayList<ServiceInfo<V>> serviceInfos = new ArrayList<>();
        List<ResolveInfo> resolveInfos = queryIntentServices(userId);
        if (forComponentChange && this.registeredServicesCacheExt.adjustGenerateServicesMap(getClass().getSimpleName(), changedUids, resolveInfos, new Predicate() { // from class: android.content.pm.RegisteredServicesCache$$ExternalSyntheticLambda1
            @Override // java.util.function.Predicate
            public final boolean test(Object obj) {
                return this.f$0.m945x491ee14f(userId, (Integer) obj);
            }
        })) {
            return;
        }
    ...
    /* renamed from: lambda$generateServicesMap$1$android-content-pm-RegisteredServicesCache, reason: not valid java name */
    /* synthetic */ boolean m945x491ee14f(int userId, Integer changedUid) {
        UserServices<V> precheckUser = findOrCreateUserLocked(userId);
        return (precheckUser.persistentServices.values().contains(changedUid) || changedUid.intValue() == -1) ? false : true;
    }
    ...
    private void readPersistentServicesLocked(InputStream is) throws XmlPullParserException, IOException, ErrnoException {
    ...
    private void writePersistentServicesLocked(UserServices<V> user, int userId) throws IOException {
        if (this.mSerializerAndParser == null) {
            return;
        }
