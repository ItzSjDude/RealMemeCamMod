...
public final class IntentFilterVerificationInfo implements Parcelable {
    ...
    public IntentFilterVerificationInfo(TypedXmlPullParser parser) throws XmlPullParserException, IOException {
        this.mDomains = new ArraySet<>();
        readFromXml(parser);
    }
    ...
    String getStringFromXml(TypedXmlPullParser parser, String attribute, String defaultValue) {
        ...
        if (value == null) {
            StringBuilder sbAppend = new StringBuilder().append("Missing element under ");
            String str = TAG;
            String msg = sbAppend.append(str).append(": ").append(attribute).append(" at ").append(parser.getPositionDescription()).toString();
            Log.w(str, msg);
            return defaultValue;
        }
        return value;
