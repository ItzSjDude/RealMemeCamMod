...
public class CallerInfo {
    ...
    public static String getGeoDescription(Context context, String number) {
        boolean z = VDBG;
        if (z) {
            Log.v(TAG, "getGeoDescription('" + Log.garbleMiddle(number) + "')...", new Object[0]);
        }
        if (TextUtils.isEmpty(number)) {
            return null;
        }
        ...
        if (z) {
            try {
                Log.v(TAG, "parsing '" + Log.garbleMiddle(number) + "' for countryIso '" + countryIso + "'...", new Object[0]);
    ...
    private static String getCurrentCountryIso(Context context, Locale locale) {
        ...
        if (countryIso == null) {
            ...
            if (VDBG) {
                Log.w(TAG, "No CountryDetector; falling back to countryIso based on locale: " + countryIso, new Object[0]);
            }
        }
        return countryIso;
    }
