...
public final class IncrementalFileStorages {
    ...
    private IncrementalFileStorages(File stageDir, File inheritedDir, IncrementalManager incrementalManager, DataLoaderParams dataLoaderParams) throws IOException {
        try {
            ...
            if (inheritedDir != null && IncrementalManager.isIncrementalPath(inheritedDir.getAbsolutePath())) {
                IncrementalStorage incrementalStorageOpenStorage = incrementalManager.openStorage(inheritedDir.getAbsolutePath());
                ...
                if (incrementalStorageOpenStorage != null) {
                    ...
                    IncrementalStorage incrementalStorageCreateStorage = incrementalManager.createStorage(stageDir.getAbsolutePath(), this.mInheritedStorage, 5);
            ...
            IncrementalStorage incrementalStorageCreateStorage2 = incrementalManager.createStorage(stageDir.getAbsolutePath(), dataLoaderParams, 5);
    ...
    private void addApkFile(InstallationFileParcel apk) throws IOException {
        ...
        if (!targetFile.exists()) {
            this.mDefaultStorage.makeFile(apkName, apk.size, null, apk.metadata, apk.signature, null);
        }
    }
    ...
    public void makeFile(String name, byte[] content) throws IOException {
        this.mDefaultStorage.makeFile(name, content.length, UUID.randomUUID(), null, null, content);
    }
