...
public final class HidlMemoryUtil {
    ...
    public static HidlMemory byteArrayToHidlMemory(byte[] input, String name) {
        ...
        try {
            ...
            try {
                ...
                HidlMemory hidlMemorySharedMemoryToHidlMemory = sharedMemoryToHidlMemory(shmem);
                if (shmem != null) {
                    shmem.close();
                }
    ...
    public static byte[] hidlMemoryToByteArray(HidlMemory mem) throws ErrnoException {
    ...
    public static ArrayList<Byte> hidlMemoryToByteList(HidlMemory mem) throws ErrnoException {
        Preconditions.checkNotNull(mem);
        Preconditions.checkArgumentInRange(mem.getSize(), 0L, 2147483647L, "Memory size");
        Preconditions.checkArgument(mem.getSize() == 0 || mem.getName().equals("ashmem"), "Unsupported memory type: %s", mem.getName());
        if (mem.getSize() == 0) {
    ...
    private static ByteBuffer getBuffer(HidlMemory mem) throws ErrnoException {
        try {
    ...
    static /* synthetic */ void lambda$getBuffer$0(long address, int size) throws ErrnoException {
        try {
