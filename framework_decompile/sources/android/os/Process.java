...
public class Process {
    ...
    public static final int ZYGOTE_POLICY_FLAG_APP32_BOOST = Integer.MIN_VALUE;
    ...
    private static boolean IS_PREVERSION = isPreVersion();
    private static boolean IS_AGING_VERSION = IOplusUIFirstManager.APP_START_ANIMATION.equals(SystemProperties.get("persist.sys.agingtest", "0"));
    ...
    public static final native void setCgroupProcsProcessGroup(int i, int i2, int i3, boolean z) throws SecurityException, IllegalArgumentException;
    ...
    static String readProcComm(int pid) {
        String[] comm = new String[1];
        if (!readProcFile("/proc/" + pid + "/comm", new int[]{4096}, comm, null, null)) {
            return null;
        }
    ...
    static String readProcCmdline(int pid) {
        String[] comm = new String[1];
        if (!readProcFile("/proc/" + pid + "/cmdline", new int[]{4096}, comm, null, null)) {
            return null;
        }
        if (!TextUtils.isEmpty(comm[0]) && comm[0].endsWith("\n")) {
            return comm[0].substring(0, comm[0].length() - 1);
    ...
    static boolean isOtherKillSystemOrZygote(int pid) {
        String cmdline = readProcCmdline(pid);
        boolean systemserver_or_zygote_group = "system_server".equals(cmdline) || Zygote.PRIMARY_SOCKET_NAME.equals(cmdline) || "zygote64".equals(cmdline) || Zygote.SECONDARY_SOCKET_NAME.equals(cmdline);
        if (!systemserver_or_zygote_group) {
            return false;
        }
        int mytid = myTid();
        int mypid = myPid();
        String comm = readProcComm(mytid);
        if (!"system_server".equals(comm) && !"watchdog".equals(comm)) {
            Throwable threadThrowable = new Throwable("kill system_server or zygote Stack trace:");
            threadThrowable.setStackTrace(Thread.currentThread().getStackTrace());
            Log.e(LOG_TAG, "kill system_server or zygote!", threadThrowable);
        }
        Log.d(LOG_TAG, "pid: " + mypid + "  tid: " + mytid + " : " + comm + " want to send sig 9 to " + pid + " :" + cmdline);
        return true;
    }

    public static final void killProcess(int pid) {
        if (IS_PREVERSION && myPid() == pid && isWeChatProcess()) {
            Log.e(LOG_TAG, " WeChat Quit ITSELF, Pid: " + pid);
            Log.e(LOG_TAG, " WeChat Quit ITSELF, StackTrace: " + Log.getStackTraceString(new Throwable()));
        }
        if (IS_AGING_VERSION) {
            isOtherKillSystemOrZygote(pid);
        }
        sendSignal(pid, 9);
    }
    ...
    public static void waitForProcessDeath(int pid, int timeout) throws InterruptedException, TimeoutException, ErrnoException {
    ...
    private static final boolean isWeChatProcess() {
        String packageName = ActivityThread.currentPackageName();
        return "com.tencent.mm".equals(packageName);
    }

    private static final boolean isPreVersion() {
        String otaVersion = SystemProperties.get("ro.build.version.ota");
        if (otaVersion != null && otaVersion.contains("PRE")) {
            return true;
        }
        return false;
    }
