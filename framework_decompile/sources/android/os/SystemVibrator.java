...
public class SystemVibrator extends Vibrator {
    ...
    private ISystemVibratorExt mSystemVibratorExt;
    ...
    public SystemVibrator(Context context) {
        ...
        this.mSystemVibratorExt = (ISystemVibratorExt) ExtLoader.type(ISystemVibratorExt.class).base(this).create();
        ...
        Log.d(TAG, "SystemVibrator Created");
    }
    ...
    public void vibrate(int uid, String opPkg, VibrationEffect effect, String reason, VibrationAttributes attributes) {
        ...
        if (vibratorManager == null) {
            ...
            if (vibratorManager.doVibrate(uid, opPkg, effect) || this.mSystemVibratorExt.convertToLinearVibration(uid, opPkg, effect, this.mToken, attributes)) {
                return;
            }
            CombinedVibration combinedEffect = CombinedVibration.createParallel(effect);
    ...
    public static class MultiVibratorInfo extends VibratorInfo {
        ...
        private static VibratorInfo.FrequencyProfile frequencyProfileIntersection(VibratorInfo[] infos) {
            float freqResolution = floatPropertyIntersection(infos, new Function() { // from class: android.os.SystemVibrator$MultiVibratorInfo$$ExternalSyntheticLambda5
                @Override // java.util.function.Function
                public final Object apply(Object obj) {
                    return Float.valueOf(((VibratorInfo) obj).getFrequencyProfile().getFrequencyResolutionHz());
                }
            });
            ...
            int amplitudeCount = Math.round(((((Float) freqRange.getUpper()).floatValue() - ((Float) freqRange.getLower()).floatValue()) / freqResolution) + 1.0f);
            ...
            for (VibratorInfo info : infos) {
                ...
                int vibratorStartIdx = Math.round((((Float) freqRange.getLower()).floatValue() - ((Float) vibratorFreqRange.getLower()).floatValue()) / freqResolution);
            ...
            return new VibratorInfo.FrequencyProfile(resonantFreq, ((Float) freqRange.getLower()).floatValue(), freqResolution, maxAmplitudes);
        }
        ...
        private static Range<Float> frequencyRangeIntersection(VibratorInfo[] infos, float frequencyResolution) {
            ...
            float intersectionLower = ((Float) firstRange.getLower()).floatValue();
            float intersectionUpper = ((Float) firstRange.getUpper()).floatValue();
            for (int i = 1; i < infos.length; i++) {
                Range<Float> vibratorRange = infos[i].getFrequencyProfile().getFrequencyRangeHz();
                if (vibratorRange == null || ((Float) vibratorRange.getLower()).floatValue() >= intersectionUpper || ((Float) vibratorRange.getUpper()).floatValue() <= intersectionLower) {
                    return null;
                }
                float frequencyDelta = Math.abs(intersectionLower - ((Float) vibratorRange.getLower()).floatValue());
                if (frequencyDelta % frequencyResolution > EPSILON) {
                    return null;
                }
                intersectionLower = Math.max(intersectionLower, ((Float) vibratorRange.getLower()).floatValue());
                intersectionUpper = Math.min(intersectionUpper, ((Float) vibratorRange.getUpper()).floatValue());
            }
    ...
    static class AllVibratorsStateListener {
    ...
    public void linearMotorVibrate(int uid, String opPkg, int[] waveformIds, long[] timings, int strength, int repeat, String reason, AudioAttributes attributes, IBinder token) {
        ...
        VibrationAttributes attr = new VibrationAttributes.Builder(attributes2).build();
