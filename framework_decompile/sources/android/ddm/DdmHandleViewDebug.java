...
public class DdmHandleViewDebug extends DdmHandle {
    ...
    private Chunk dumpHierarchy(View rootView, ByteBuffer in) {
        ...
        try {
            ...
            return new Chunk(CHUNK_VURT, (byte[]) byteArray, 0, byteArray.length);
        } catch (IOException | InterruptedException e) {
            return createFailChunk(byteArray, "Unexpected error while obtaining view hierarchy: " + e.getMessage());
        }
    }
    ...
    private Chunk captureView(View rootView, View targetView) throws InterruptedException {
        ByteArrayOutputStream b = new ByteArrayOutputStream(1024);
        try {
    ...
    private Chunk invokeViewMethod(View rootView, View targetView, ByteBuffer in) throws NoSuchMethodException, SecurityException {
        Class<?>[] argTypes;
        Object[] args;
        int l = in.getInt();
    ...
    private Chunk profileView(View rootView, View targetView) throws IOException {
        ...
        try {
            ...
            Chunk chunkCreateFailChunk = createFailChunk(1, "Unexpected error while profiling view: " + e2.getMessage());
            try {
                bw.close();
            } catch (IOException e3) {
            }
