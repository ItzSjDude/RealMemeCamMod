...
public class ValueAnimator extends Animator implements AnimationHandler.AnimationFrameCallback {
    ...
    public static void setDurationScale(float durationScale) {
        ...
        for (WeakReference<DurationScaleChangeListener> listenerRef : listenerCopy) {
            DurationScaleChangeListener listener = listenerRef.get();
            if (listener != null) {
    ...
    public static boolean unregisterDurationScaleChangeListener(DurationScaleChangeListener listener) {
        ...
        synchronized (arrayList) {
            ...
            zRemove = sDurationScaleChangeListeners.remove(listenerRefToRemove);
        }
        return zRemove;
    }
    ...
    boolean animateBasedOnTime(long currentTime) {
        ...
        if (this.mRunning) {
            ...
            float fClampFraction = clampFraction(fraction);
            this.mOverallFraction = fClampFraction;
            float currentIterationFraction = getCurrentIterationFraction(fClampFraction, this.mReversing);
            animateValue(currentIterationFraction);
        }
        return done;
    }
    ...
    void animateBasedOnPlayTime(long currentPlayTime, long lastPlayTime, boolean inReverse) {
        ...
        if (i > 0) {
            long j = this.mDuration;
            int iteration = (int) (currentPlayTime / j);
            int lastIteration = (int) (lastPlayTime / j);
            if (Math.min(iteration, i) != Math.min(lastIteration, this.mRepeatCount) && this.mListeners != null) {
        ...
        if (iteration2 != -1 && currentPlayTime >= (iteration2 + 1) * this.mDuration) {
            skipToEndValue(inReverse);
        } else {
            float fraction = currentPlayTime / this.mDuration;
    ...
    public final boolean doAnimationFrame(long frameTime) {
        long jResolveDurationScale;
        if (this.mStartTime < 0) {
            if (this.mReversing) {
                jResolveDurationScale = frameTime;
            } else {
                jResolveDurationScale = ((long) (this.mStartDelay * resolveDurationScale())) + frameTime;
            }
            this.mStartTime = jResolveDurationScale;
        }
        if (this.mPaused) {
    ...
    public ValueAnimator mo54clone() {
        ValueAnimator anim = (ValueAnimator) super.mo54clone();
        if (this.mUpdateListeners != null) {
        ...
        if (oldValues != null) {
            ...
            for (int i = 0; i < numValues; i++) {
                PropertyValuesHolder newValuesHolder = oldValues[i].mo99clone();
