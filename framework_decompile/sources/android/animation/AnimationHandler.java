...
public class AnimationHandler {
    ...
    private Choreographer.FrameCallback mPauser = new Choreographer.FrameCallback() { // from class: android.animation.AnimationHandler$$ExternalSyntheticLambda0
        ...
        public final void doFrame(long j) {
            this.f$0.m52lambda$new$0$androidanimationAnimationHandler(j);
        }
    };
    ...
    private void removeRequestorImpl(Object requestor) {
        ...
        this.mAnimatorRequestors.remove(requestor);
    }
    ...
    private void requestAnimatorsEnabledImpl(boolean enable, Object requestor) {
        ...
        if (enable) {
            this.mAnimatorRequestors.add(requestor);
        } else {
            this.mAnimatorRequestors.remove(requestor);
        }
    ...
    /* renamed from: lambda$new$0$android-animation-AnimationHandler, reason: not valid java name */
    /* synthetic */ void m52lambda$new$0$androidanimationAnimationHandler(long frameTimeNanos) {
        if (this.mAnimatorRequestors.size() > 0) {
            return;
        }
        for (int i = 0; i < this.mAnimationCallbacks.size(); i++) {
            Animator animator = (Animator) this.mAnimationCallbacks.get(i);
            if (animator != null && animator.getTotalDuration() == -1 && !animator.isPaused()) {
