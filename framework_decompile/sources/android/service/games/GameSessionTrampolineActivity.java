...
public final class GameSessionTrampolineActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            startActivityAsCaller((Intent) getIntent().getParcelableExtra(INTENT_KEY), getIntent().getBundleExtra(OPTIONS_KEY), false, getUserId(), 1);
        } catch (Exception e) {
            Slog.w(TAG, "Unable to launch activity from game session");
            AndroidFuture<GameSessionActivityResult> future = (AndroidFuture) getIntent().getParcelableExtra(FUTURE_KEY);
