...
public class ZenModeConfig implements Parcelable {
    ...
    public ZenPolicy toZenPolicy() {
        int zenPolicySenders;
        int zenPolicySenders2;
        int zenPolicySenders3;
        ZenPolicy.Builder builder = new ZenPolicy.Builder();
        if (this.allowCalls) {
            zenPolicySenders = getZenPolicySenders(this.allowCallsFrom);
        } else {
            zenPolicySenders = 4;
        }
        ZenPolicy.Builder builderAllowRepeatCallers = builder.allowCalls(zenPolicySenders).allowRepeatCallers(this.allowRepeatCallers);
        if (this.allowMessages) {
            zenPolicySenders2 = getZenPolicySenders(this.allowMessagesFrom);
        } else {
            zenPolicySenders2 = 4;
        }
        ZenPolicy.Builder builderAllowSystem = builderAllowRepeatCallers.allowMessages(zenPolicySenders2).allowReminders(this.allowReminders).allowEvents(this.allowEvents).allowAlarms(this.allowAlarms).allowMedia(this.allowMedia).allowSystem(this.allowSystem);
        if (this.allowConversations) {
            zenPolicySenders3 = getZenPolicySenders(this.allowConversationsFrom);
        } else {
            zenPolicySenders3 = 4;
        }
        ZenPolicy.Builder builder2 = builderAllowSystem.allowConversations(zenPolicySenders3);
        int i = this.suppressedVisualEffects;
        if (i == 0) {
            builder2.showAllVisualEffects();
        } else {
            builder2.showFullScreenIntent((i & 4) == 0);
    ...
    public static Condition toTimeCondition(Context context, long time, int minutes, int userHandle, boolean shortVersion) throws Resources.NotFoundException {
        String line2;
        String line1;
        String line22;
        ...
        if (minutes < 60) {
            ...
            line2 = PluralsMessageFormatter.format(res, arguments, summaryResId);
            int line1ResId = shortVersion ? R.string.zen_mode_duration_minutes_short : R.string.zen_mode_duration_minutes;
            ...
            arguments.put("formattedTime", formattedTime);
            line2 = PluralsMessageFormatter.format(res, arguments, summaryResId2);
            int line1ResId2 = shortVersion ? R.string.zen_mode_duration_hours_short : R.string.zen_mode_duration_hours;
        ...
        return new Condition(id, line2, line1, line22, 0, 1, 1);
    }

    public static Condition toNextAlarmCondition(Context context, long alarm, int userHandle) throws Resources.NotFoundException {
    ...
    public static EventInfo tryParseEventConditionId(Uri conditionId) {
        boolean isEvent = conditionId != null && Condition.SCHEME.equals(conditionId.getScheme()) && "android".equals(conditionId.getAuthority()) && conditionId.getPathSegments().size() == 1 && "event".equals(conditionId.getPathSegments().get(0));
        if (!isEvent) {
            return null;
        }
    ...
    private static String getConditionLine(Context context, ZenModeConfig config, int userHandle, boolean useLine1, boolean shortVersion) throws Resources.NotFoundException {
        if (config == null) {
            return "";
        }
        String summary = "";
