...
public class AudioRecord implements AudioRouting, MicrophoneDirection, AudioRecordingMonitor, AudioRecordingMonitorClient {
    ...
    private AudioRecord(AudioAttributes attributes, AudioFormat format, int bufferSizeInBytes, int sessionId, Context context, int maxSharedAudioHistoryMs) throws Throwable {
        ...
        this.mAudioRecordExt = (IAudioRecordExt) ExtLoader.type(IAudioRecordExt.class).create();
    ...
    public static int getMinBufferSize(int sampleRateInHz, int channelConfig, int audioFormat) {
        ...
        switch (channelConfig) {
            ...
            case 28:
                channelCount = 3;
                break;
            case AudioFormat.CHANNEL_IN_5POINT1 /* 1507580 */:
                channelCount = 6;
                break;
            default:
    ...
    public void startRecording(MediaSyncEvent syncEvent) throws IllegalStateException {
        ...
        if (this.mAudioRecordExt.permInterceptInStartRecordingWithEvent()) {
            throw new IllegalStateException("permission denied");
        }
        synchronized (this.mRecordingStateLock) {
    ...
    @SystemApi
    public MediaSyncEvent shareAudioHistory(String sharedPackage, long startFromMillis) throws IllegalArgumentException {
    ...
    private int native_setup(Object audiorecordThis, Object attributes, int[] sampleRate, int channelMask, int channelIndexMask, int audioFormat, int buffSizeInBytes, int[] sessionId, String opPackageName, long nativeRecordInJavaObj) {
        ...
        AttributionSource.ScopedParcelState attributionSourceState = attributionSource.asScopedParcelState();
        try {
            int iNative_setup = native_setup(audiorecordThis, attributes, sampleRate, channelMask, channelIndexMask, audioFormat, buffSizeInBytes, sessionId, attributionSourceState.getParcel(), nativeRecordInJavaObj, 0);
            if (attributionSourceState != null) {
                attributionSourceState.close();
            }
