...
public class ImageWriter implements AutoCloseable {
    ...
    private void initializeImageWriter(Surface surface, int maxImages, boolean useSurfaceImageFormatInfo, boolean useLegacyImageFormat, int imageFormat, int hardwareBufferFormat, int dataSpace, int width, int height, long usage) {
        int imageFormat2;
        if (surface != null && maxImages >= 1) {
            ...
            if (!useSurfaceImageFormatInfo) {
                ...
                imageFormat2 = SurfaceUtils.getSurfaceFormat(surface);
                this.mHardwareBufferFormat = PublicFormatUtils.getHalFormat(imageFormat2);
                this.mDataSpace = PublicFormatUtils.getHalDataspace(imageFormat2);
            }
            if (imageFormat2 == 33) {
                ...
                switch (surfaceDataspace) {
                    ...
                    default:
                        ...
                        break;
                    case 4098:
                        imageFormat2 = ImageFormat.DEPTH_JPEG;
                        break;
                    case 4099:
                        imageFormat2 = ImageFormat.HEIC;
                        break;
                }
                this.mHardwareBufferFormat = PublicFormatUtils.getHalFormat(imageFormat2);
                this.mDataSpace = PublicFormatUtils.getHalDataspace(imageFormat2);
            ...
            this.mEstimatedNativeAllocBytes = ImageUtils.getEstimatedNativeAllocBytes(this.mWidth, height2, useLegacyImageFormat ? imageFormat2 : hardwareBufferFormat, 1);
    ...
    private ImageWriter(Surface surface, int maxImages, boolean useSurfaceImageFormatInfo, int imageFormat, int width, int height) {
        ...
        initializeImageWriter(surface, maxImages, useSurfaceImageFormatInfo, true, imageFormat, this.mHardwareBufferFormat, halDataspace, width, height, this.mUsage);
    }
    ...
    private ImageWriter(Surface surface, int maxImages, boolean useSurfaceImageFormatInfo, int imageFormat, int width, int height, long usage) {
        ...
        int halDataspace = PublicFormatUtils.getHalDataspace(imageFormat);
        this.mDataSpace = halDataspace;
        initializeImageWriter(surface, maxImages, useSurfaceImageFormatInfo, true, imageFormat, this.mHardwareBufferFormat, halDataspace, width, height, usage);
    }
    ...
    private ImageWriter(Surface surface, int maxImages, boolean useSurfaceImageFormatInfo, int hardwareBufferFormat, int dataSpace, int width, int height, long usage) {
        ...
        this.mUsage = usage;
        if (useSurfaceImageFormatInfo) {
            this.mHardwareBufferFormat = PublicFormatUtils.getHalFormat(0);
            this.mDataSpace = PublicFormatUtils.getHalDataspace(0);
            imageFormat = 0;
        } else {
        ...
        initializeImageWriter(surface, maxImages, useSurfaceImageFormatInfo, false, imageFormat, hardwareBufferFormat, dataSpace, width, height, usage);
    }
    ...
    private void attachAndQueueInputImage(Image image) {
        ...
        if (image.getNativeContext() != 0) {
            nativeAttachAndQueueImage(this.mNativeContext, image.getNativeContext(), image.getFormat(), image.getTimestamp(), image.getDataSpace(), crop.left, crop.top, crop.right, crop.bottom, image.getTransform(), image.getScalingMode());
            return;
        }
        GraphicBuffer gb = GraphicBuffer.createFromHardwareBuffer(image.getHardwareBuffer());
        nativeAttachAndQueueGraphicBuffer(this.mNativeContext, gb, image.getFormat(), image.getTimestamp(), image.getDataSpace(), crop.left, crop.top, crop.right, crop.bottom, image.getTransform(), image.getScalingMode());
        gb.destroy();
        image.close();
    }
    ...
    private static class WriterSurfaceImage extends Image {
        ...
        private native synchronized SurfacePlane[] nativeCreatePlanes(int i, int i2, int i3);
        ...
        public Image.Plane[] getPlanes() {
            ...
            if (this.mPlanes == null) {
                ...
                this.mPlanes = nativeCreatePlanes(numPlanes, getOwner().getFormat(), getOwner().getDataSpace());
            }
