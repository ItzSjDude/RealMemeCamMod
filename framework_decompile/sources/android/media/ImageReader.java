...
public class ImageReader implements AutoCloseable {
    ...
    private void initializeImageReader(int width, int height, int imageFormat, int maxImages, long usage, int hardwareBufferFormat, int dataSpace, boolean useLegacyImageFormat) {
        if (width < 1 || height < 1) {
        ...
        this.mEstimatedNativeAllocBytes = ImageUtils.getEstimatedNativeAllocBytes(width, height, useLegacyImageFormat ? imageFormat : hardwareBufferFormat, 1);
    ...
    private ImageReader(int width, int height, int imageFormat, int maxImages, long usage, MultiResolutionImageReader parent) {
        ...
        initializeImageReader(width, height, imageFormat, maxImages, usage, halFormat, halDataspace, true);
    }
    ...
    private ImageReader(int width, int height, int maxImages, long usage, MultiResolutionImageReader parent, int hardwareBufferFormat, int dataSpace) {
        ...
        initializeImageReader(width, height, 0, maxImages, usage, hardwareBufferFormat, dataSpace, false);
    }
    ...
    public Image acquireNextImageNoThrowISE() {
        ...
        if (this.mUseLegacyImageFormat) {
            si = new SurfaceImage(this.mFormat);
        } else {
            si = new SurfaceImage(this.mHardwareBufferFormat, this.mDataSpace);
        }
    ...
    private int acquireNextSurfaceImage(SurfaceImage si) {
        ...
        synchronized (this.mCloseLock) {
            ...
            if (this.mIsReaderValid) {
                status = nativeImageSetup(si, this.mUseLegacyImageFormat);
            }
            switch (status) {
                case 0:
    ...
    private class SurfaceImage extends Image {
        ...
        private native synchronized HardwareBuffer nativeGetOplusHardwareBuffer();
