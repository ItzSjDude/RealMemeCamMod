...
public class MediaMetrics {
    ...
    public static class Item {
        ...
        public Item(String key, int pid, int uid, long timeNs, int capacity) {
            ...
            this.mKey = key;
            ByteBuffer byteBufferAllocateDirect = ByteBuffer.allocateDirect(Math.max(capacity, i + 4));
            this.mBuffer = byteBufferAllocateDirect;
            byteBufferAllocateDirect.order(ByteOrder.nativeOrder()).putInt(0).putInt(i).putChar((char) 0).putChar((char) (keyLength + 1)).put(keyBytes).put((byte) 0).putInt(pid).putInt(uid).putLong(timeNs);
