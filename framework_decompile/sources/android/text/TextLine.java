...
public class TextLine {
    ...
    private final ITextLineExt mTextLineExt = (ITextLineExt) ExtLoader.type(ITextLineExt.class).base(this).create();
    private ITextLineWrapper mTextLineWrapper = new TextLineWrapper();
    ...
    */
        ...
        if (runIndex >= 0) {
            if (offset != (after ? this.mLen : 0)) {
                ...
                if (this.mSpanned == null || runStart == runLimit) {
                    ...
                    while (true) {
                        int iNextSpanTransition = this.mSpanned.nextSpanTransition(this.mStart + spanStart2, limit, MetricAffectingSpan.class);
                        i = this.mStart;
                        spanLimit2 = iNextSpanTransition - i;
    ...
    private float handleRun(int start, int measureLimit, int limit, boolean runIsRtl, Canvas c, TextShaper.GlyphsConsumer consumer, float x, int top, int y, int bottom, Paint.FontMetricsInt fmi, boolean needWidth) {
        ...
        while (i5 < measureLimit) {
            ...
            if (replacement != null) {
                ...
                while (true) {
                    ...
                    if (j3 == i5) {
                        ...
                        if (needWidth) {
                            ...
                            decorationInfo = decorationInfo2;
                            i = i5;
                            x3 += handleText(activePaint2, activeStart3, activeEnd, i5, inext, runIsRtl, c, consumer, x3, top, y, bottom, fmi, z, Math.min(activeEnd, mlimit), textLine2.mDecorations);
                            wp = wp2;
                            activePaint = activePaint2;
                            activePaint.set(wp);
                            textLine = this;
                ...
                x2 = x3 + handleText(activePaint3, activeStart4, activeEnd, i10, inext, runIsRtl, c, consumer, x3, top, y, bottom, fmi, needWidth || activeEnd < measureLimit, Math.min(activeEnd, mlimit), textLine3.mDecorations);
            }
            i5 = inext;
        }
        return x2 - x;
    ...
    public ITextLineWrapper getWrapper() {
        return this.mTextLineWrapper;
    }

    private class TextLineWrapper implements ITextLineWrapper {
        private TextLineWrapper() {
        }

        @Override // android.text.ITextLineWrapper
        public void setAddedWidthForJustify(float value) {
            TextLine.this.mAddedWidthForJustify = value;
        }

        @Override // android.text.ITextLineWrapper
        public void setIsJustifying(boolean isJustifying) {
            TextLine.this.mIsJustifying = isJustifying;
        }
    }
}
