...
public class PropertyInvalidatedCache<Query, Result> {
    ...
    public static void setTestMode(boolean mode) {
        sTesting = mode;
        HashMap<String, Long> map = sTestingPropertyMap;
        synchronized (map) {
            map.clear();
        }
    }
    ...
    private long getCurrentNonce() {
        if (sTesting) {
            HashMap<String, Long> map = sTestingPropertyMap;
            synchronized (map) {
                Long n = map.get(this.mPropertyName);
                if (n != null) {
                    return n.longValue();
                }
            }
    ...
    public Result query(Query query) {
        ...
        while (!isReservedNonce(currentNonce)) {
            ...
            if (result != null) {
                        ...
                        synchronized (this.mLock) {
                            if (currentNonce == this.mLastSeenNonce) {
                                if (resultRefresh == null) {
                                    ...
                                    this.mCache.put(query, resultRefresh);
                                }
                            }
                        }
                ...
                synchronized (this.mLock) {
                    if (this.mLastSeenNonce == currentNonce && resultRecompute != null) {
                        this.mCache.put(query, resultRecompute);
                    }
                    this.mMisses++;
                }
    ...
    public static void invalidateCache(String name) {
        ...
        synchronized (sCorkLock) {
            ...
            if (numberCorks != null && numberCorks.intValue() > 0) {
                HashMap<String, Long> map = sCorkedInvalidates;
                long count = map.getOrDefault(name, 0L).longValue();
                map.put(name, Long.valueOf(1 + count));
                return;
            }
    ...
    */
    ...
    public static void uncorkInvalidations(String name) {
        ...
        synchronized (sCorkLock) {
            HashMap<String, Integer> map = sCorks;
            int numberCorks = map.getOrDefault(name, 0).intValue();
            if (numberCorks < 1) {
            ...
            if (numberCorks == 1) {
                ...
                map.put(name, Integer.valueOf(numberCorks - 1));
            }
        }
    }
    ...
    private void dumpContents(PrintWriter pw, boolean detailed, String[] args) throws Throwable {
        ...
        synchronized (obj2) {
            try {
                try {
                    ...
                    long[] jArr = this.mSkips;
                    long skips = jArr[2] + jArr[0] + jArr[1] + jArr[3];
    ...
    private static void dumpCacheInfo(PrintWriter pw, String[] args) throws Throwable {
