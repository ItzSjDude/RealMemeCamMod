...
public final class RulesState implements Parcelable {
    ...
    public RulesState(String baseRulesVersion, DistroFormatVersion distroFormatVersionSupported, boolean operationInProgress, int stagedOperationType, DistroRulesVersion stagedDistroRulesVersion, int distroStatus, DistroRulesVersion installedDistroRulesVersion) {
        ...
        int iValidateStagedOperation = validateStagedOperation(stagedOperationType);
        this.mStagedOperationType = iValidateStagedOperation;
        this.mStagedDistroRulesVersion = (DistroRulesVersion) Utils.validateConditionalNull(iValidateStagedOperation == 3, "stagedDistroRulesVersion", stagedDistroRulesVersion);
        int iValidateDistroStatus = validateDistroStatus(distroStatus);
        this.mDistroStatus = iValidateDistroStatus;
        this.mInstalledDistroRulesVersion = (DistroRulesVersion) Utils.validateConditionalNull(iValidateDistroStatus == 2, "installedDistroRulesVersion", installedDistroRulesVersion);
    }
    ...
    public int hashCode() {
        int iHashCode;
        int iHashCode2 = ((((((this.mBaseRulesVersion.hashCode() * 31) + this.mDistroFormatVersionSupported.hashCode()) * 31) + (this.mOperationInProgress ? 1 : 0)) * 31) + this.mStagedOperationType) * 31;
        DistroRulesVersion distroRulesVersion = this.mStagedDistroRulesVersion;
        if (distroRulesVersion != null) {
            iHashCode = distroRulesVersion.hashCode();
        } else {
            iHashCode = 0;
        }
        int i = (((iHashCode2 + iHashCode) * 31) + this.mDistroStatus) * 31;
        DistroRulesVersion distroRulesVersion2 = this.mInstalledDistroRulesVersion;
        return i + (distroRulesVersion2 != null ? distroRulesVersion2.hashCode() : 0);
    }
