...
class FileSynthesisCallback extends AbstractSynthesisCallback {
    ...
    @Override // android.speech.tts.SynthesisCallback
    public int start(int sampleRateInHz, int audioFormat, int channelCount) throws IOException {
    ...
    @Override // android.speech.tts.SynthesisCallback
    public int audioAvailable(byte[] buffer, int offset, int length) throws IOException {
        synchronized (this.mStateLock) {
            int i = this.mStatusCode;
            if (i == -2) {
                return errorCodeOnStop();
