...
public class Scroller {
        ...
        if (timePassed < i) {
            switch (this.mMode) {
                ...
                case 1:
                    ...
                    int iRound = this.mStartX + Math.round((this.mFinalX - r1) * distanceCoef);
                    this.mCurrX = iRound;
                    int iMin = Math.min(iRound, this.mMaxX);
                    this.mCurrX = iMin;
                    this.mCurrX = Math.max(iMin, this.mMinX);
                    int iRound2 = this.mStartY + Math.round((this.mFinalY - r1) * distanceCoef);
                    this.mCurrY = iRound2;
    ...
    public void fling(int startX, int startY, int velocityX, int velocityY, int minX, int maxX, int minY, int maxY) {
        ...
        this.mMinY = minY;
        this.mMaxY = maxY;
        int iRound = ((int) Math.round(coeffX * totalDistance)) + startX;
        this.mFinalX = iRound;
        int iMin = Math.min(iRound, this.mMaxX);
        this.mFinalX = iMin;
        this.mFinalX = Math.max(iMin, this.mMinX);
        int iRound2 = ((int) Math.round(coeffY * totalDistance)) + startY;
