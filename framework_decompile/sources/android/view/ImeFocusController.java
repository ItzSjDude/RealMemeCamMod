...
public final class ImeFocusController {
    ...
    private static boolean sDebugIme = false;
    ...
    private InputMethodManagerDelegate getImmDelegate() {
        if (this.mDelegate == null) {
            ...
            if (DEBUG) {
                Log.v(TAG, "getImmDelegate display = " + this.mViewRootImpl.mContext.getDisplayId() + "/" + inputMethodManager.getDisplayId() + ", " + this.mViewRootImpl);
            }
        }
    ...
    public boolean checkFocus(boolean forceNewFocus, boolean startInput) {
        ...
        if (DEBUG) {
            StringBuilder sbAppend = new StringBuilder().append("checkFocus: view=").append(this.mServedView).append(" next=").append(this.mNextServedView).append(" force=").append(forceNewFocus).append(" package=");
            View view = this.mServedView;
            Log.v(TAG, sbAppend.append(view != null ? view.getContext().getPackageName() : "<none>").toString());
        }
    ...
    void onViewFocusChanged(View view, boolean hasFocus) {
        ...
        if (!getImmDelegate().isCurrentRootView(view.getViewRootImpl())) {
            ...
            if (sDebugIme && !hasFocus) {
                Log.d(TAG, "onViewFocusChanged callers: " + Debug.getCallers(15));
            }
            if (hasFocus) {
