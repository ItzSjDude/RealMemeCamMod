...
public final class MotionEvent extends InputEvent implements Parcelable {
    ...
    private boolean initialize(int deviceId, int source, int displayId, int action, int flags, int edgeFlags, int metaState, int buttonState, int classification, float xOffset, float yOffset, float xPrecision, float yPrecision, long downTimeNanos, long eventTimeNanos, int pointerCount, PointerProperties[] pointerIds, PointerCoords[] pointerCoords) {
        long jNativeInitialize = nativeInitialize(this.mNativePtr, deviceId, source, displayId, action, flags, edgeFlags, metaState, buttonState, classification, xOffset, yOffset, xPrecision, yPrecision, downTimeNanos, eventTimeNanos, pointerCount, pointerIds, pointerCoords);
    ...
    public final MotionEvent clampNoHistory(float left, float top, float right, float bottom) throws Throwable {
        MotionEvent ev = obtain();
        synchronized (gSharedTempLock) {
            try {
                int pointerCount = nativeGetPointerCount(this.mNativePtr);
    ...
    public final MotionEvent split(int idBits) {
        ...
        synchronized (gSharedTempLock) {
            ...
            int historySize2 = nativeGetHistorySize(ev2.mNativePtr);
            int h2 = 0;
            while (h2 <= historySize2) {
                ...
                if (h2 == 0) {
                    int iNativeGetDeviceId = nativeGetDeviceId(ev2.mNativePtr);
                    int iNativeGetSource = nativeGetSource(ev2.mNativePtr);
                    int iNativeGetDisplayId = nativeGetDisplayId(ev2.mNativePtr);
                    int iNativeGetFlags = nativeGetFlags(ev2.mNativePtr);
                    int h3 = h2;
                    int iNativeGetEdgeFlags = nativeGetEdgeFlags(ev2.mNativePtr);
                    int historySize3 = historySize2;
                    int iNativeGetMetaState = nativeGetMetaState(ev2.mNativePtr);
                    int newPointerCount3 = newPointerCount;
                    int iNativeGetButtonState = nativeGetButtonState(ev2.mNativePtr);
                    int oldActionPointerIndex3 = oldActionPointerIndex;
                    int iNativeGetClassification = nativeGetClassification(ev2.mNativePtr);
                    oldActionMasked2 = oldActionMasked;
                    float fNativeGetXOffset = nativeGetXOffset(ev2.mNativePtr);
                    oldAction2 = oldAction;
                    float fNativeGetYOffset = nativeGetYOffset(ev2.mNativePtr);
                    float fNativeGetXPrecision = nativeGetXPrecision(ev2.mNativePtr);
                    float fNativeGetYPrecision = nativeGetYPrecision(ev2.mNativePtr);
                    long jNativeGetDownTimeNanos = nativeGetDownTimeNanos(ev2.mNativePtr);
                    h = h3;
                    historySize = historySize3;
                    newPointerCount2 = newPointerCount3;
                    oldActionPointerIndex2 = oldActionPointerIndex3;
                    ev = ev3;
                    ev3.initialize(iNativeGetDeviceId, iNativeGetSource, iNativeGetDisplayId, newAction, iNativeGetFlags, iNativeGetEdgeFlags, iNativeGetMetaState, iNativeGetButtonState, iNativeGetClassification, fNativeGetXOffset, fNativeGetYOffset, fNativeGetXPrecision, fNativeGetYPrecision, jNativeGetDownTimeNanos, eventTimeNanos, newPointerCount2, pp, pc);
                } else {
                    h = h2;
