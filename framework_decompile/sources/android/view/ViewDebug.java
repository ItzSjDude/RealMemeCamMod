...
public class ViewDebug {
    ...
    static void dispatchCommand(View view, String command, String parameters, OutputStream clientStream) throws InterruptedException, IOException {
    ...
    private static long profileViewOperation(View view, final ViewOperation operation) {
        ...
        view.post(new Runnable() { // from class: android.view.ViewDebug$$ExternalSyntheticLambda8
            @Override // java.lang.Runnable
            public final void run() {
                ViewDebug.lambda$profileViewOperation$3(operation, duration, latch);
            }
        });
        try {
    ...
    private static void captureViewLayer(View view, DataOutputStream clientStream, boolean visible) throws InterruptedException, IOException {
    ...
    private static void capture(View root, OutputStream clientStream, String parameter) throws InterruptedException, IOException {
        View captureView = findView(root, parameter);
        capture(root, clientStream, captureView);
    }

    public static void capture(View root, OutputStream clientStream, View captureView) throws InterruptedException, IOException {
    ...
    private static Bitmap performViewCapture(final View captureView, final boolean skipChildren) throws InterruptedException {
        if (captureView != null) {
            final CountDownLatch latch = new CountDownLatch(1);
            ...
            captureView.post(new Runnable() { // from class: android.view.ViewDebug$$ExternalSyntheticLambda0
                @Override // java.lang.Runnable
                public final void run() {
                    ViewDebug.lambda$performViewCapture$4(captureView, cache, skipChildren, latch);
                }
            });
            try {
    ...
    public static void dumpv2(final View view, ByteArrayOutputStream out) throws InterruptedException, IOException {
        ...
        for (int attributeId : attributes) {
            try {
                ...
                if (!theme.resolveAttribute(attributeId, outValue, true)) {
                    ...
                    string = outValue.coerceToString().toString();
                }
                data[i2] = string;
                i += 2;
                if (outValue.type == 1) {
    ...
    private static void dumpViewHierarchy(final Context context, final ViewGroup group, final BufferedWriter out, final int level, final boolean skipChildren, final boolean includeProperties) throws ExecutionException, InterruptedException, TimeoutException, IOException {
    ...
    private static void cacheExportedProperties(Class<?> klass) {
        ...
        if (map != null && map.containsKey(klass)) {
            return;
        }
        do {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    /* JADX WARN: Multi-variable type inference failed */
    public static void dumpViewHierarchyOnUIThread(Context context, ViewGroup viewGroup, BufferedWriter out, int level, boolean skipChildren, boolean includeProperties) throws IOException {
    ...
    private static boolean dumpView(Context context, View view, BufferedWriter out, int level, boolean includeProperties) throws IOException {
    ...
    private static <T extends Annotation> PropertyInfo<T, ?>[] convertToPropertyInfos(Method[] methods, Field[] fields, final Class<T> property) {
        return (PropertyInfo[]) Stream.of((Object[]) new Stream[]{Arrays.stream(methods).map(new Function() { // from class: android.view.ViewDebug$$ExternalSyntheticLambda1
            ...
            public final Object apply(Object obj) {
                return ViewDebug.PropertyInfo.forMethod((Method) obj, property);
            }
        }), Arrays.stream(fields).map(new Function() { // from class: android.view.ViewDebug$$ExternalSyntheticLambda2
            @Override // java.util.function.Function
            public final Object apply(Object obj) {
                return ViewDebug.PropertyInfo.forField((Field) obj, property);
            }
    ...
    private static void writeExportedProperties(Context context, Object view, BufferedWriter out, Class<?> klass, String prefix) throws IOException {
        ...
        for (PropertyInfo<ExportedProperty, ?> info : getExportedProperties(klass)) {
            try {
                ...
                categoryPrefix = ((ExportedProperty) info.property).category().length() != 0 ? ((ExportedProperty) info.property).category() + ":" : "";
            } catch (Exception e) {
            }
            if (info.returnType == Integer.TYPE || info.returnType == Byte.TYPE) {
                if (((ExportedProperty) info.property).resolveId() && context != null) {
                    int id = ((Integer) value).intValue();
                    value = resolveId(context, id);
                } else if (((ExportedProperty) info.property).formatToHexString()) {
                    ...
                    IntToString[] mapping = ((ExportedProperty) info.property).mapping();
                    if (mapping.length > 0) {
                ...
                if (info.returnType == int[].class) {
                    ...
                    exportUnrolledArray(context, out, (ExportedProperty) info.property, (int[]) value, valuePrefix2, info.entrySuffix);
                } else if (info.returnType == String[].class) {
                    String[] array = (String[]) value;
                    if (((ExportedProperty) info.property).hasAdjacentMapping() && array != null) {
                ...
                } else if (!info.returnType.isPrimitive() && ((ExportedProperty) info.property).deepExport()) {
                    dumpViewProperties(context, value, out, prefix + ((ExportedProperty) info.property).prefix());
                }
            }
    ...
    public static Object invokeViewMethod(final View view, final Method method, final Object[] args) throws InterruptedException {
        final CountDownLatch latch = new CountDownLatch(1);
    ...
    public static void setLayoutParameter(final View view, String param, int value) throws IllegalAccessException, NoSuchFieldException, IllegalArgumentException {
