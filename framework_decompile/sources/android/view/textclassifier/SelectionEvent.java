...
public final class SelectionEvent implements Parcelable {
    ...
    public static SelectionEvent createSelectionActionEvent(int start, int end, int actionType) throws IllegalArgumentException {
        Preconditions.checkArgument(end >= start, "end cannot be less than start");
        checkActionType(actionType);
        return new SelectionEvent(start, end, actionType, "", 0, "");
    }

    public static SelectionEvent createSelectionActionEvent(int start, int end, int actionType, TextClassification classification) throws IllegalArgumentException {
        String entityType;
        Preconditions.checkArgument(end >= start, "end cannot be less than start");
        Objects.requireNonNull(classification);
        checkActionType(actionType);
