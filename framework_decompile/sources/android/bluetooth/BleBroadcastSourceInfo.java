...
public final class BleBroadcastSourceInfo implements Parcelable {
    ...
    BleBroadcastSourceInfo(BluetoothDevice audioSource, byte advSid, int addressType, int metadataSyncstate, int audioSyncstate, List<BleBroadcastSourceChannel> selectedBISIndicies) {
        this.mAudioBisIndexList = new HashMap();
        this.mMetadataList = new HashMap();
        ...
        if (selectedBISIndicies == null) {
            ...
            while (i2 < selectedBISIndicies.size()) {
                if (selectedBISIndicies.get(i2).getStatus()) {
                    ...
                    BASS_Debug(TAG, "audioBisIndex" + audioBisIndex3);
                    this.mAudioBisIndexList.put(Integer.valueOf(subGroupId), audioBisIndex3);
                }
                i2++;
                i = 0;
    ...
    public BleBroadcastSourceInfo(BluetoothDevice device, byte sourceId, byte advSid, int broadcasterId, int addressType, int metadataSyncstate, int audioSyncstate, int encryptionStatus, String broadcastCode, byte[] badCode, byte numSubGroups, Map<Integer, Integer> bisIndiciesList, Map<Integer, byte[]> metadataList) {
        this.mAudioBisIndexList = new HashMap();
        this.mMetadataList = new HashMap();
        this.mSourceId = sourceId;
        this.mMetaDataSyncState = metadataSyncstate;
    ...
    void setAudioSyncState(int audioSyncState) {
        this.mAudioSyncState = audioSyncState;
    }
    ...
    void setBroadcastChannelsSyncStatus(List<BleBroadcastSourceChannel> selectedBISIndicies) {
        if (selectedBISIndicies == null) {
            BASS_Debug(TAG, "selectedBISIndiciesList is null");
            return;
        }
    ...
    static void BASS_Debug(String TAG2, String msg) {
        if (BASS_DBG) {
            Log.d(TAG2, msg);
        }
    }
