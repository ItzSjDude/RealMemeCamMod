...
import com.oplus.ocs.camera.consumer.apsAdapter.adapter.ApsTotalResult;
import com.oplus.ocs.camera.consumer.apsAdapter.service.ApsAdapterListener;
import com.oplus.ocs.camera.producer.info.ApsCameraMetadataKey;
import com.oplus.ocs.camera.producer.info.ApsOtherParameter;
import com.oplus.ocs.camera.producer.info.CameraConstant;
import com.oplus.ocs.camera.producer.info.DefaultUtill;
import com.oplus.ocs.camera.common.util.ContextHolder;
import com.oplus.ocs.camera.common.util.PlatformUtil;
import com.oplus.statistics.StatisticsManager;
import com.oplus.ocs.camera.producer.info.Parameter;
import com.oplus.ocs.camera.producer.info.SurfaceKey;
import com.oplus.ocs.camera.producer.info.SurfacePool;
import com.oplus.ocs.camera.appinterface.CameraImage;
import com.oplus.ocs.camera.producer.info.ConfigInfoPkg;
import com.oplus.ocs.camera.common.util.CameraHandlerThread;
import com.oplus.ocs.camera.common.util.CameraPerformance;
import com.oplus.ocs.camera.configure.ApsRequestTag;
import com.oplus.ocs.camera.producer.info.ApsCameraRequestTag;
...
public class ConsumerImpl
    ...
    public void onPreviewImageArrived(CameraRequestTag cameraRequestTag, @NonNull ImageReader imageReader,
        ...
        if (isPreviewImageNeedApsProcessor()) {
            if (SurfacePool.getInstance().getImageReader(surfaceKey) != null) {
                this.mProcessorContract.addPreview(imageReader, imageAcquireNextImage,
                        ImageCategory.ItemInfoType.PREVIEW, cameraType, surfaceKey);
            } else {
            ...
            ((CameraPreviewCallbackAdapter) cameraRequestTag.mCallback)
                    .onPreviewReceived(new CameraPreviewCallbackAdapter.PreviewResult.Builder()
                            .setImageBuffer(new ApsResult.ImageBuffer(null, imageAcquireNextImage,
                                    Util.getHardwareBuffer(imageAcquireNextImage),
                                    imageAcquireNextImage.getTimestamp()))
        ...
        try {
            ...
            try {
                if (isNeedPreviewMeta(null)) {
                    ...
                    this.mProcessorContract.addPreviewMeta(captureRequest, imageReader, imageAcquireNextImage,
            ...
            CameraUnitLog.e(TAG, "onPreviewMetaImageArrived", e2);
        }
    }
    ...
    public void onPreviewReceived(final ApsResult apsResult, final ApsTotalResult apsTotalResult) {
        ...
        if (zIsNeedApsProcessor && cameraRequestTag != null
                && TextUtils.equals("0", this.mApsTag.mPreviewCallbackType)) {
            final CameraPreviewCallbackAdapter.PreviewResult previewResultBuild = new CameraPreviewCallbackAdapter.PreviewResult.Builder()
                    .setCaptureResult(apsTotalResult.getTotalResult()).setApsResult(apsResult)
        ...
        this.mPreviewReceivedWorkHandler.post(new Runnable() { // from class:
            ...
            @Override // java.lang.Runnable
            public final void run() {
                ConsumerImpl.this.m5x8c39f4ed(zIsNeedApsProcessor, apsTotalResult, cameraRequestTag, apsResult,
                        currentRequest);
            }
        });
    ...
    /*
     * JADX INFO: renamed from:
     * lambda$onPreviewReceived$1$com-oplus-ocs-camera-consumer-ConsumerImpl,
     * reason: not valid java name
     */
    /* synthetic */ void m5x8c39f4ed(boolean z, ApsTotalResult apsTotalResult, CameraRequestTag cameraRequestTag,
     ...
     */
    /* synthetic */ void m7xee08042b(ImageReader imageReader, SurfaceKey surfaceKey, CameraRequestTag cameraRequestTag,
        ...
        try {
            ...
            if (PlatformUtil.isQualcommPlatform()) {
                ...
                } else if ("rear_sat" == cameraRequestTag.mCameraType || "rear_portrait" == cameraRequestTag.mCameraType
                ...
                switch (cameraRequestTag.mCameraType) {
                    ...
                    case "rear_portrait":
                        if (!"rear_wide".equals(surfaceKey.getCameraType())
                                && !CameraConstant.CameraType.REAR_MONO_CAMERA_1.equals(surfaceKey.getCameraType())) {
                            i = 0;
                            break;
                        } else {
                            i = 2;
                ...
                if (imageAcquireNextImage == null) {
                    CameraUnitLog.e(TAG,
                            "pictureProcess, role: " + i + ", format: " + imageAcquireNextImage.getFormat()
                                    + ", timestamp: " + imageAcquireNextImage.getTimestamp() + ", size: "
                                    + imageAcquireNextImage.getWidth() + "x" + imageAcquireNextImage.getHeight());
                    onHalPictureImageArrived(imageReader2, imageAcquireNextImage, i, cameraRequestTag,
                            surfaceKey.getUsage());
                    return;
                }
                return;
        ...
        if (imageReader != null) {
            imageReader.setOnImageAvailableListener(new ImageReader.OnImageAvailableListener() { // from class:
                ...
                public void onImageAvailable(ImageReader imageReader2) {
                    ...
                    if (imageAcquireNextImage != null) {
                        ApsResult.ImageBuffer imageBuffer = new ApsResult.ImageBuffer(imageReader2,
                                imageAcquireNextImage, Util.getHardwareBuffer(imageAcquireNextImage),
                                imageAcquireNextImage.getTimestamp());
                        CameraUnitLog.d(ConsumerImpl.TAG, "tuningDataProcess, format: "
                                + imageAcquireNextImage.getFormat() + ", timestamp: "
                                + imageAcquireNextImage.getTimestamp() + ", size: " + imageAcquireNextImage.getWidth()
                                + "x" + imageAcquireNextImage.getHeight() + "surfaceKey.getCameraType： "
                                + surfaceKey.getCameraType() + ", tag.mCameraType：" + cameraRequestTag.mCameraType
                        ...
                        if ("rear_wide".equals(surfaceKey.getCameraType())) {
                            if (36 == cameraRequestTag.mApsDecisionSceneMode
                                ...
                                && "rear_sat".equals(cameraRequestTag.mCameraType)
                                && 2 == cameraRequestTag.mSupportCaptureZoomFeature) {
    ...
    private void notifyPreviewResult(Handler handler, final CameraPreviewCallbackAdapter cameraPreviewCallbackAdapter,
        ...
        synchronized (this.mRequestLock) {
            handler.post(new Runnable() { // from class:
                                          // com.oplus.ocs.camera.consumer.ConsumerImpl$$ExternalSyntheticLambda1
                @Override // java.lang.Runnable
                public final void run() {
