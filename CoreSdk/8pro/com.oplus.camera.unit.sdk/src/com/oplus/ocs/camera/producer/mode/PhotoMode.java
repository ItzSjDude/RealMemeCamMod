...
public class PhotoMode extends BaseMode {
    ...
    protected void onConfigure(CameraSessionEntity cameraSessionEntity, SdkCameraDeviceConfig sdkCameraDeviceConfig,
        ...
        if ("rear_main".equals(str) || "rear_wide".equals(str) || "rear_tele".equals(str)) {
            if (CameraCharacteristicsHelper.getCameraIdType("rear_sat") == null
                    && CameraConfigHelper.getUseCaseValues("standard_photo_case") != null) {
        ...
        apsRequestTag.mbHighPictureSizeEnable = "on"
                .equals(sdkCameraDeviceConfig.getConfigureParameter().get(ConfigureParameter.HIGH_PICTURE_SIZE_ENABLE));
        ...
        if ("front_main".equals(str)
            ...
            if (pictureSurfaces.size() > 0) {
                ...
                Size sizeGeMaxSize = geMaxSize(CameraConfigBase.KEY_FRONT_MFNR_PICTURE_SIZE,
                        ((double) halSurfaceSize.getWidth()) / ((double) halSurfaceSize.getHeight()));
                CameraUnitLog.v(TAG, "onConfigure, cameraType: " + str + ", pictureSize: " + halSurfaceSize
                        + ", mfnrSize: " + sizeGeMaxSize);
                if (sizeGeMaxSize != null && halSurfaceSize.getHeight() <= sizeGeMaxSize.getHeight()
                        && halSurfaceSize.getWidth() <= sizeGeMaxSize.getWidth()) {
    ...
    protected boolean checkPreviewResult(CameraRequestTag cameraRequestTag) {
        ...
        if (apsRequestTag == null || !"basic_capture_mode".equals(apsRequestTag.mModeName)
    ...
    public CameraRequestTag createRequestTag(String str, Object obj, Handler handler, String str2,
        ...
        synchronized (this.mPreviewResultLock) {
            if (this.mPreviewResult != null && Parameter.ParameterStage.BEFORE_TAKE_PICTURE.equals(str2)) {
                ...
                cameraRequestTagCreateRequestTag.mbAIShutter = ((Boolean) this.mPreviewResult
                        .get(ApsDecisionParameter.KEY_PREVIEW_AI_SHUTTER)).booleanValue();
                cameraRequestTagCreateRequestTag.mbEeyEnhance = AlgoSwitchConfig.getSupportCaptureAlgo(
                        cameraRequestTagCreateRequestTag.mCaptureMode, cameraRequestTagCreateRequestTag.mCameraId,
                        ParameterKeys.ALGO_NAME_EYE_ENHANCE) && 1 == cameraRequestTagCreateRequestTag.mRearFrontCameraId
                        && !cameraRequestTagCreateRequestTag.mbInThirdApp;
                if (Util.isHdrOn(this.mPreviewResult) && !cameraRequestTagCreateRequestTag.mbBurstShot) {
                    cameraRequestTagCreateRequestTag.mCaptureEvList = (int[]) this.mPreviewResult
                            .get(ApsDecisionParameter.KEY_PREVIEW_CAPTURE_EV_LIST);
                    cameraRequestTagCreateRequestTag.mCaptureEtList = (long[]) this.mPreviewResult
                            .get(ApsDecisionParameter.KEY_PREVIEW_CAPTURE_ET_LIST);
                }
                if (builder.get(PreviewParameter.KEY_ZOOM_RATIO) != null) {
                    cameraRequestTagCreateRequestTag.mZoomRatio = ((Float) builder.get(PreviewParameter.KEY_ZOOM_RATIO))
                            .floatValue();
                }
                if (cameraRequestTagCreateRequestTag.mbBurstShot) {
                    int iIntValue = ((Integer) CameraConfigHelper
                            .getConfigValue(CameraConfigBase.KEY_CSHOT_FIRST_REQUEST_NUM, 3)).intValue();
                ...
                if (((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_SE_NIGHT_SUPPORT, false))
                        .booleanValue() && Util.getNightStateDecision(this.mPreviewResult) > 0
                        && !cameraRequestTagCreateRequestTag.mbBurstShot) {
                    cameraRequestTagCreateRequestTag.mbInNightProcess = true;
                }
                cameraRequestTagCreateRequestTag.mAISState = ((Integer) this.mPreviewResult
                        .get(ApsDecisionParameter.KEY_PREVIEW_AIS_STATE)).intValue();
                cameraRequestTagCreateRequestTag.mAsdMovingObject = ((Integer) this.mPreviewResult
                        .get(ApsDecisionParameter.KEY_PREVIEW_ASD_MOVING_OBJECT)).intValue();
                cameraRequestTagCreateRequestTag.mRequestSensorMode = ((Integer) this.mPreviewResult
                        .get(ApsDecisionParameter.KEY_PREVIEW_SENSOR_MODE)).intValue();
                if (isApsAlgoHdr(cameraRequestTagCreateRequestTag)
                        || isApsAlgoLowLightHdr(cameraRequestTagCreateRequestTag)) {
                    String str3 = (String) builder.get(PreviewParameter.KEY_HDR_MODE);
                    cameraRequestTagCreateRequestTag.mHdrMode = str3;
                    cameraRequestTagCreateRequestTag.mbHdrTrigger = "on".equals(str3)
                            || ("auto".equals(str3) && Util.isHdrOn(this.mPreviewResult));
                }
                ...
                if (builder.containCustomKey(PreviewParameter.KEY_NIGHT_SE_ENABLE)) {
                    cameraRequestTagCreateRequestTag.mNightSeEnable = ((Boolean) builder
                            .get(PreviewParameter.KEY_NIGHT_SE_ENABLE)).booleanValue();
                }
            }
        }
    ...
    private boolean isApsAlgoLowLightHdr(CameraRequestTag cameraRequestTag) {
        if ("1".equals(CameraConfigHelper.getConfigValue(DefaultUtill.KEY_REAR_LOWLIGHT_HDR_SUPPORT))
                && cameraRequestTag.mApsAlgoFlags != null) {
            for (String str : cameraRequestTag.mApsAlgoFlags) {
    ...
    public void updateStageParameterBuilder(@NonNull PreviewParameter.Builder builder, String str, String str2,
        ...
        switch (str) {
            ...
            case "start_recording":
                ...
                return;
            case "configure":
                if (((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_TELE_SMALL_PREVIEW_SUPPORT,
                        false)).booleanValue() && PlatformUtil.isMtkPlatform() && getModeName().equals("photo_mode")) {
                    Size size = this.mTagMap.get(str2).mPreviewSize;
                    Size smallSizeByFormat = CameraCharacteristicsHelper.getSmallSizeByFormat(str2,
                            ((double) size.getWidth()) / ((double) size.getHeight()), 35);
            ...
            case "start_preview":
                ...
                if (cameraRequestTag != null) {
                    ...
                    if (builder.containCustomKey(PreviewParameter.KEY_FACE_DERED_EYE_ENABLE)) {
                        cameraRequestTag.mbDeRedEye = "on"
                                .equals(builder.get(PreviewParameter.KEY_FACE_DERED_EYE_ENABLE));
                    }
                    if (builder.containCustomKey(PreviewParameter.KEY_NIGHT_SE_ENABLE)) {
                        cameraRequestTag.mNightSeEnable = ((Boolean) builder.get(PreviewParameter.KEY_NIGHT_SE_ENABLE))
                                .booleanValue();
                    }
                    if (builder.containCustomKey(PreviewParameter.KEY_PREVIEW_IN_PREVIEW_ENABLE)) {
                        this.mbPreviewInPreview = "on"
                                .equals(builder.get(PreviewParameter.KEY_PREVIEW_IN_PREVIEW_ENABLE));
                    } else {
                        this.mbPreviewInPreview = false;
                    }
                    if (((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_SUPPORT_IZOOM_PREVIEW_ENABLE,
                            false)).booleanValue()) {
                        synchronized (this.mPreviewResultLock) {
                    ...
                    synchronized (this.mPreviewResultLock) {
                        if (this.mPreviewResult != null) {
                            int iIntValue = ((Integer) this.mPreviewResult
                                    .get(ApsDecisionParameter.KEY_PREVIEW_SENSOR_MODE)).intValue();
    ...
    public void updateStageParameter(@NonNull Parameter parameter, String str, String str2,
        ...
        if (Parameter.ParameterStage.BEFORE_TAKE_PICTURE.equals(str)) {
            Boolean bool = (Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_TUNING_DATA_BUFFER_SUPPORT,
                    false);
            if (PlatformUtil.isMtkPlatform() && cameraRequestTag != null
                    && CameraCharacteristicsHelper.isFrontCamera(cameraRequestTag.mCameraId)
                    && cameraRequestTag.mbInNightProcess && cameraRequestTag.mCaptureEvList != null
                    && Util.isSupportOfflineNight(getModeName())) {
            ...
            if (bool != null && bool.booleanValue() && cameraRequestTag != null) {
                ...
                parameter.set(PreviewParameter.KEY_TUNING_DATA_REQUEST,
                        CameraRequestTag.RequestMode.CAPTURE_RAW == cameraRequestTag.getRequestMode() ? new byte[] { 1 }
                                : new byte[] { 2 });
                if (32 == cameraRequestTag.mRequestFormat) {
                    parameter.set((Parameter.Key<Integer>) PreviewParameter.KEY_CAPTURE_PROCESS_RAW_ENABLE, 1);
                    ...
                    parameter.set((Parameter.Key<Integer>) PreviewParameter.KEY_CAPTURE_PROCESS_RAW_ENABLE, 0);
                    parameter.set(PreviewParameter.KEY_CAPTURE_RAW_BITS_PER_PIXEL, new int[] { 12 });
                }
            }
            if (cameraRequestTag != null) {
                parameter.set(PreviewParameter.FEATURE_AI_SHUT_CAPTURE,
                        new int[] { cameraRequestTag.mbAIShutter ? 1 : 0 });
            }
            ...
            if (PlatformUtil.isMtkPlatform() && cameraRequestTag != null && "rear_sat".equals(str2)
                    && cameraRequestTag.mSensorSizes == null) {
                cameraRequestTag.mSensorSizes = getCaptureSizeArrayByCameraType(str2);
            }
            synchronized (this.mPreviewResultLock) {
                if (PlatformUtil.isMtkPlatform()) {
                    int iIntValue = this.mPreviewResult != null
                            ? ((Integer) this.mPreviewResult.get(ApsDecisionParameter.KEY_PREVIEW_FEATURE_TYPE))
                                    .intValue()
                            : 0;
                    if (cameraRequestTag.mbNeedPreviewStream != null && cameraRequestTag.mbNeedPreviewStream.length > 0
                            && 50 == iIntValue) {
    ...
    public Size[] getCaptureSizeArrayByCameraType(String str) {
        ...
        if ("rear_sat".equals(str)) {
            return new Size[] { geMaxSize(CameraConfigBase.KEY_SAT_SUB_PICTURE_SIZE, width),
                    geMaxSize(CameraConfigBase.KEY_SAT_MAIN_PICTURE_SIZE, width),
    ...
    public Pair<Size, Size> getSurfaceSize(SdkCameraDeviceConfig sdkCameraDeviceConfig, String str, String str2,
        ...
        switch (str) {
            ...
            case "capture_raw_dol":
                size2 = size;
                return new Pair<>(pictureSurfaces.size() <= 0 ? pictureSurfaces.get(0).getAppSurfaceSize() : size2,
                    ...
                    switch (str2) {
                        case "rear_main":
                            Size size4 = ((Size[]) CameraConfigHelper
                                    .getConfigValue(CameraConfigBase.KEY_SAT_FUSION_MAIN_RAW_SIZE, null))[0];
                            return new Pair<>(size4, size4);
                        case "rear_tele":
                            Size size5 = ((Size[]) CameraConfigHelper
                                    .getConfigValue(CameraConfigBase.KEY_SAT_FUSION_TELE_RAW_SIZE, null))[0];
                            return new Pair<>(size5, size5);
                        case "rear_wide":
                            Size size6 = ((Size[]) CameraConfigHelper
                                    .getConfigValue(CameraConfigBase.KEY_SAT_FUSION_WIDE_RAW_SIZE, null))[0];
                            return new Pair<>(size6, size6);
                        default:
                            return new Pair<>(
                                    pictureSurfaces.size() <= 0 ? pictureSurfaces.get(0).getAppSurfaceSize() : size2,
                                    CameraCharacteristicsHelper.getSizeByFormat(str3, 1.3333333333333333d, 32));
                    }
                }
                return new Pair<>(pictureSurfaces.size() > 0 ? pictureSurfaces.get(0).getAppSurfaceSize() : size2,
            ...
            case "reprocess_yuv":
                ...
                if ("rear_wide".equals(str2) && (size3 = (Size) CameraConfigHelper
                        .getConfigValue(CameraConfigBase.KEY_HIGH_PICTURE_SIZE_REAR_WIDE, null)) != null) {
                    return new Pair<>(size3, size3);
                }
                if ((CameraConstant.CameraType.REAR_MONO_CAMERA_1.equals(str3)
                        || CameraConstant.CameraType.REAR_MONO_CAMERA_2.equals(str3)) && "rear_main".equals(str2)) {
    ...
    protected boolean needAddToTarget(String str, String str2, @NonNull SurfaceKey surfaceKey,
        ...
        if (Parameter.ParameterStage.START_PREVIEW.equals(str2)) {
            if ("on".equals(builder.get(PreviewParameter.KEY_PREVIEW_IN_PREVIEW_ENABLE))
                    && "surface_key_preview_in_preview".equals(surfaceKey.getUsage())) {
                return true;
            }
        } else if (Parameter.ParameterStage.BEFORE_TAKE_PICTURE.equals(str2)) {
            if (isVideoSnapShotByAps(str) && "on".equals(builder.get(PreviewParameter.KEY_QUICK_VIDEO_RECORD_ENABLE))
            ...
            boolean z = (this.mPreviewResult == null || zEquals || (32 != ((Integer) this.mPreviewResult
                    .get(ApsDecisionParameter.KEY_PREVIEW_REQUEST_FORMAT)).intValue()
                    && 34 != ((Integer) this.mPreviewResult.get(ApsDecisionParameter.KEY_PREVIEW_REQUEST_FORMAT))
                            .intValue()
                    && 36 != ((Integer) this.mPreviewResult.get(ApsDecisionParameter.KEY_PREVIEW_REQUEST_FORMAT))
                            .intValue())) ? false : true;
            if (this.mPreviewResult != null) {
                int iIntValue = ((Integer) this.mPreviewResult.get(ApsDecisionParameter.KEY_PREVIEW_MASTER_PIPELINE))
                        .intValue();
                ...
                if ((Util.isAIOrPoitraitNight(this.mPreviewResult) || Util.isTurboHdr(this.mPreviewResult)) && z) {
                    if ("surface_key_picture_dol".equals(surfaceKey.getUsage())) {
                        return 50 == (this.mPreviewResult != null
                                ? ((Integer) this.mPreviewResult.get(ApsDecisionParameter.KEY_PREVIEW_FEATURE_TYPE))
                                        .intValue()
                                : 0) && surfaceKey.getFormat() == 32;
                    }
    ...
    protected boolean needRemoveTarget(String str, String str2, @NonNull SurfaceKey surfaceKey,
        ...
        if (Parameter.ParameterStage.START_PREVIEW.equals(str2)
                && "surface_key_preview_in_preview".equals(surfaceKey.getUsage())
                && (!builder.containCustomKey(PreviewParameter.KEY_PREVIEW_IN_PREVIEW_ENABLE)
                        || "off".equals(builder.get(PreviewParameter.KEY_PREVIEW_IN_PREVIEW_ENABLE)))) {
            return true;
        }
    ...
    public boolean needStartPreview(CameraPreviewCallbackAdapter.PreviewResult previewResult) {
        ...
        if (((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_SUPPORT_IZOOM_PREVIEW_ENABLE, false))
                .booleanValue()
                && this.mZoomFeaturePrev != ((Integer) previewResult
                        .get(ApsDecisionParameter.KEY_PREVIEW_SUPPORT_ZOOM_FEATURE)).intValue()) {
            int iIntValue2 = ((Integer) previewResult.get(ApsDecisionParameter.KEY_PREVIEW_SUPPORT_ZOOM_FEATURE))
                    .intValue();
    ...
    private boolean needInputSurface(String str) {
        return PlatformUtil.isQualcommPlatform()
                && !((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_DISABLE_RAW, false))
                        .booleanValue()
                && AlgoSwitchConfig.getSupportCaptureAlgo(
                        ApsDataConvert.getApsModeName(getModeName(), this.mRearFrontCameraId), this.mRearFrontCameraId,
                        ParameterKeys.ALGO_NAME_SUPERNIGHT);
    }
    ...
    private void updateCaptureHdrMode(@NonNull String str, @NonNull PreviewParameter.Builder builder, String str2) {
        ...
        if ("auto".equals(str2) || "off".equals(str2)) {
            builder.set((CaptureRequest.Key<Integer>) CaptureRequest.CONTROL_SCENE_MODE, 0);
        } else if ("on".equals(str2)) {
            builder.set((CaptureRequest.Key<Integer>) CaptureRequest.CONTROL_SCENE_MODE, 18);
        }
    ...
    protected void updatePreviewRequestTag(PreviewParameter.Builder builder, CameraRequestTag cameraRequestTag,
        ...
        if (!builder.containCustomKey(PreviewParameter.KEY_FOCUS_MODE)
                && cameraDeviceInfo.isSupportPreviewParameter(PreviewParameter.KEY_FOCUS_MODE.getName())) {
            builder.set((Parameter.Key<Integer>) PreviewParameter.KEY_FOCUS_MODE, 1);
        }
        synchronized (this.mVideoEisOnLock) {
            if (cameraRequestTag != null) {
                if ("on".equals(builder.get(PreviewParameter.KEY_QUICK_VIDEO_RECORD_ENABLE))
                        && !CameraConstant.MonoFilterType.BLACK_WHITE.equals(this.mTagMap.get(str2).mFeatureName)) {
                    this.mbVideoEisOn = false;
    ...
    private void updateCameraRequestTag(PreviewParameter.Builder builder, CameraRequestTag cameraRequestTag) {
        ...
        if (cameraRequestTag.mbBurstShot) {
            int iIntValue = ((Integer) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_CSHOT_FIRST_REQUEST_NUM,
                    3)).intValue();
        ...
        if (builder.containCustomKey(PreviewParameter.KEY_TILT_SHIFT_PARAM)
                && (bundle = (Bundle) builder.get(PreviewParameter.KEY_TILT_SHIFT_PARAM)) != null) {
            ...
            cameraRequestTag.mTiltShiftCenterPosition = bundle
                    .getString(CameraConstant.TiltShiftParamKeys.CENTER_POSITION, "");
            cameraRequestTag.mTiltShiftClearDistance = bundle
                    .getString(CameraConstant.TiltShiftParamKeys.CLEAR_DISTANCE, "");
        ...
        synchronized (this.mPreviewResultLock) {
            if (!cameraRequestTag.mbBurstShot && this.mPreviewResult != null) {
                ...
                cameraRequestTag.mApsBracketMode = ((Integer) this.mPreviewResult
                        .get(ApsDecisionParameter.KEY_PREVIEW_BRACKET_MODE)).intValue();
                cameraRequestTag.mAsdSceneValue = ((Integer) this.mPreviewResult
                        .get(ApsDecisionParameter.KEY_PREVIEW_ASD_SCENE_VALUE)).intValue();
        ...
        CameraUnitLog.d(TAG, "updateCaptureRequestTag, mbCaptureSloganOpen: " + cameraRequestTag.mbPhotoWaterMarkEnable
                + ", mFilterType: " + cameraRequestTag.mFilterType + ", mbFilterOpen: " + cameraRequestTag.mbFilterOpen
                + ", tag.mbPiEnable: " + cameraRequestTag.mbPiEnable + ", mMakeupType: " + cameraRequestTag.mMakeupType
                + ", tag.mSupportCaptureZoomFeature: " + cameraRequestTag.mSupportCaptureZoomFeature);
    }
    ...
    protected long[] getFlexibleCapabilities(String str) {
        ...
        boolean z = ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_TELE_SMALL_PREVIEW_SUPPORT,
                false)).booleanValue() && PlatformUtil.isMtkPlatform() && getModeName().equals("photo_mode")
                && !"front_main".equals(str) && !"front_wide".equals(str)
                && ((Boolean) getConfigureParameter(str).get(ConfigureParameter.KEY_IS_FROM_MAIN_MENU)).booleanValue();
        Size size = this.mTagMap.get(str).mPreviewSize;
        double width = ((double) size.getWidth()) / ((double) size.getHeight());
    ...
    protected long getHalMemory() {
        ...
        if (this.mbTenBitOpen) {
            int iIntValue2 = ((Integer) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_10BIT, 0))
                    .intValue();
            if (CameraConstant.MEMORY_GB_8G < totalMemory) {
                iIntValue = ((Integer) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_10BIT_12G,
                        Integer.valueOf(iIntValue2))).intValue();
            } else if (CameraConstant.MEMORY_GB_6G < totalMemory) {
                iIntValue = ((Integer) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_10BIT_8G,
                        Integer.valueOf(iIntValue2))).intValue();
            } else {
                if (CameraConstant.MEMORY_GB_4G >= totalMemory) {
                    return iIntValue2;
                }
                iIntValue = ((Integer) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_10BIT_6G,
                        Integer.valueOf(iIntValue2))).intValue();
            }
        } else if (((Boolean) CameraConfigHelper
                .getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_COMMON_SUPPORT_DIFFERENCE, false)).booleanValue()) {
            int iIntValue3 = ((Integer) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_COMMON, 0))
                    .intValue();
            if (CameraConstant.MEMORY_GB_8G < totalMemory) {
                iIntValue = ((Integer) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_COMMON_12G,
                        Integer.valueOf(iIntValue3))).intValue();
            } else if (CameraConstant.MEMORY_GB_6G < totalMemory) {
                iIntValue = ((Integer) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_COMMON_8G,
                        Integer.valueOf(iIntValue3))).intValue();
            } else {
                if (CameraConstant.MEMORY_GB_4G >= totalMemory) {
                    return iIntValue3;
                }
                iIntValue = ((Integer) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_COMMON_6G,
                        Integer.valueOf(iIntValue3))).intValue();
            }
        } else {
            iIntValue = ((Integer) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_HAL_MEMORY_COMMON, 0))
                    .intValue();
        }
        return iIntValue;
    }
    ...
    public int getSurfaceFormatBySurfaceType(String str, String str2, String str3) {
        if ("capture_yuv".equals(str) || "reprocess_yuv".equals(str)
        ...
        if ("capture_raw".equals(str)
                && ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_SE_NIGHT_SUPPORT, false))
                        .booleanValue()) {
            return 32;
        }
    ...
    protected int getRawFormat(String str) {
        if (((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_SE_NIGHT_SUPPORT, false)).booleanValue()
                || Util.isAIOrPoitraitNight(this.mPreviewResult) || Util.isTurboHdr(this.mPreviewResult)
                || isHybridRaw(this.mPreviewResult)) {
            return 32;
        }
        if (((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_CAPTURE_WITH_RAW10, false)).booleanValue()
    ...
    public int isAllowedToTakePicture(CameraPictureCallbackAdapter cameraPictureCallbackAdapter, Handler handler,
        ...
        boolean z;
        int iIsAllowedToTakePicture = super.isAllowedToTakePicture(cameraPictureCallbackAdapter, handler,
                cameraRequestTag);
        if (iIsAllowedToTakePicture == 0 && "photo_mode".equals(getModeName())) {
            ...
            while (true) {
                ...
                if (currentSensorMode == cameraRequestTag.mRequestSensorMode
                        || !(3 == currentSensorMode || 3 == cameraRequestTag.mRequestSensorMode)) {
                    break;
                }
                try {
                    Thread.sleep(30L);
            ...
            CameraUnitLog.v(TAG,
                    "isAllowedToTakePicture, wait for seamless switch end, cost " + (((long) i2) * 30)
                            + "ms, currentSensorMode is " + currentSensorMode + ", requestSensorMode is "
                            + cameraRequestTag.mRequestSensorMode,
                    true);
            if (currentSensorMode != cameraRequestTag.mRequestSensorMode
                    && (3 == currentSensorMode || 3 == cameraRequestTag.mRequestSensorMode)) {
                CameraUnitLog.e(TAG,
                        "isAllowedToTakePicture, wait for request sensor mode(" + cameraRequestTag.mRequestSensorMode
                                + ") timeout, current sensor mode is " + currentSensorMode + ", fail!");
                z = false;
                i = 3;
            }
            if (!z && cameraPictureCallbackAdapter != null && handler != null) {
                ...
                StatisticsManager.getInstance()
                        .reportFunctionalError(StatisticConstant.FunctionalErrorKeys.KEY_CAPTURE_FAILED, 25);
