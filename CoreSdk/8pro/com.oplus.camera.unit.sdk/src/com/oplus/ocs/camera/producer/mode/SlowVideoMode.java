...
public class SlowVideoMode extends VideoMode {
    ...
    protected boolean isPreviewProcessByAps(Parameter parameter, String str) {
        return ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_SLOW_VIDEO_PREVIEW_PROCESS_BY_APS, true)).booleanValue();
    }
    ...
    public void updateStageParameterBuilder(@NonNull PreviewParameter.Builder builder, String str, String str2, @Nullable CameraRequestTag cameraRequestTag) {
        ...
        if (str.equals(Parameter.ParameterStage.CONFIGURE)) {
            if (this.mConfigMap.get(str2) != null) {
                builder.set(ConfigureParameter.KEY_MTK_SMVR_FEATURE_SMVR_MODES, CameraCharacteristicsHelper.getAvailableSmvrModes(str2, this.mConfigMap.get(str2).getVideoSurface().getAppSurfaceSize().getHeight(), getFpsValueFromString((String) builder.get(ConfigureParameter.VIDEO_FPS))));
            }
            builder.set(ConfigureParameter.KEY_SESSION_REQUEST_FAST_LAUNCH, new int[]{0});
        } else if (str.equals(Parameter.ParameterStage.START_PREVIEW) && !builder.containsKey(PreviewParameter.KEY_FOCUS_MODE)) {
            builder.set((Parameter.Key<int>) PreviewParameter.KEY_FOCUS_MODE, 3);
        }
    }
    ...
    protected void onConfigure(CameraSessionEntity cameraSessionEntity, SdkCameraDeviceConfig sdkCameraDeviceConfig, String str, ApsRequestTag apsRequestTag) {
        ...
        boolean zEquals = "on".equals(configureParameter.get(ConfigureParameter.KEY_SLOW_VIDEO_INTELLIGENT_HIGH_FRAME_ENABLE));
    ...
    public void updateFpsRange(Parameter parameter, PreviewParameter.Builder builder, String str) {
        if (!super.updateFpsRangeByVideoType(parameter, builder, str) && this.mbIntelligentHighFrameSelected) {
            ...
            builder.set((CaptureRequest.Key<Range>) CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE, new Range(Integer.valueOf(i), Integer.valueOf(i)));
        }
    }
