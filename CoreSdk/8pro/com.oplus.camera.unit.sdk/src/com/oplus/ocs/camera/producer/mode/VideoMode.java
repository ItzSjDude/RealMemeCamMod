...
public class VideoMode extends BaseMode {
    ...
    public boolean useOplusCameraCase(String str) {
        ...
        if ((configureParameter.containCustomKey(ConfigureParameter.VIDEO_FPS) && "video_60fps".equals(configureParameter.get(ConfigureParameter.VIDEO_FPS))) || isExplorerOpen(str)) {
            return true;
        }
    ...
    protected void onConfigure(CameraSessionEntity cameraSessionEntity, SdkCameraDeviceConfig sdkCameraDeviceConfig, String str, ApsRequestTag apsRequestTag) {
        ...
        if (!useOplusCameraCase(str)) {
            ...
            if (isRearSuperEisOpen(str)) {
                Map<String, String> mapSingletonMap = Collections.singletonMap(ConfigureParameter.VIDEO_STABILIZATION_MODE.getName(), "super_stabilization");
                sdkCameraDeviceConfig.updateHalPreviewSize(getCameraDeviceInfo(str).getPreviewMappingSizesFromConfig(str, mapSingletonMap));
                sdkCameraDeviceConfig.updateHalVideoSize(getCameraDeviceInfo(str).getVideoMappingSizesFromConfig(str, mapSingletonMap));
            }
            SurfaceWrapper videoSurface = sdkCameraDeviceConfig.getVideoSurface();
            if (videoSurface != null) {
                StatisticsManager.getInstance().setVideoSize(videoSurface.getAppSurfaceSize());
            }
            StatisticsManager.getInstance().setFeature(ConfigureParameter.VIDEO_STABILIZATION_MODE, (String) configureParameter.get(ConfigureParameter.VIDEO_STABILIZATION_MODE));
            StatisticsManager.getInstance().setFeature(ConfigureParameter.VIDEO_3HDR_MODE, (String) configureParameter.get(ConfigureParameter.VIDEO_3HDR_MODE));
        }
        ...
        if (!Util.isSystemCamera() && configureParameter.containCustomKey(ConfigureParameter.PREVIEW_FACE_BEAUTY_LEVEL)) {
            apsRequestTag.mPreviewFaceBeautyLevel = ((Integer) configureParameter.get(ConfigureParameter.PREVIEW_FACE_BEAUTY_LEVEL)).intValue();
        }
        ...
        switch (str) {
            ...
            case "configure":
                ...
                if (configureParameter.containCustomKey(ConfigureParameter.AI_NIGHT_VIDEO_MODE)) {
                    ...
                    boolean zBooleanValue = ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_THIRD_PARY_HVXSHDR_SUPPORT, true)).booleanValue();
                    if (zBooleanValue) {
                        ...
                        CameraUnitLog.d(TAG, "updateStageParameterBuilder, NotNeedThirdHVXSPackage: " + str4 + " isThirdHVXSSupported: " + zBooleanValue);
                    }
                    if ((("rear_main".equals(str2) && ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_REAR_HVXSHDR_SUPPORT, false)).booleanValue()) || ("front_main".equals(str2) && ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_FRONT_HVXSHDR_SUPPORT, false)).booleanValue())) && (Util.isSystemCamera() || zBooleanValue)) {
                    ...
                    if (1 != num.intValue() || (!z && ("front_main".equals(str2) || "front_wide".equals(str2)))) {
                        builder.set((Parameter.Key<int>) ConfigureParameter.AI_NIGHT_VIDEO_MODE, 0);
                    } else {
                        builder.set((Parameter.Key<int>) ConfigureParameter.AI_NIGHT_VIDEO_MODE, 1);
                    }
                } else {
                    builder.set((Parameter.Key<int>) ConfigureParameter.AI_NIGHT_VIDEO_MODE, 0);
                }
                ...
                if (PlatformUtil.isQualcommPlatform()) {
                    builder.set((Parameter.Key<int>) ConfigureParameter.KEY_VIDEO_EIS_RECORD_STATE, 0);
                } else if (!"front_main".equals(str2) || ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_SUPPORT_VIDEO_FRONT_EIS_RECORD, false)).booleanValue()) {
                    builder.set((Parameter.Key<int>) ConfigureParameter.KEY_VIDEO_EIS_RECORD_STATE, 1);
                }
                ...
                if (((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_SUPPORT_VIDEO_SUPER_EIS_PROCESS_ONLY_WIDE, false)).booleanValue()) {
                    if (!configureParameter.containCustomKey(ConfigureParameter.VIDEO_STABILIZATION_MODE)) {
                        builder.set((CaptureRequest.Key<int>) CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE, 0);
                    } else if (!"super_stabilization".equals((String) configureParameter.get(ConfigureParameter.VIDEO_STABILIZATION_MODE))) {
                        builder.set((CaptureRequest.Key<int>) CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE, 0);
                    } else {
                        builder.set((CaptureRequest.Key<int>) CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE, 1);
                    }
                    break;
                }
                break;
            ...
            case "start_preview":
                ...
                if (!builder.containsKey(PreviewParameter.KEY_FOCUS_MODE) && cameraDeviceInfo.isSupportPreviewParameter(PreviewParameter.KEY_FOCUS_MODE.getName())) {
                    builder.set((Parameter.Key<int>) PreviewParameter.KEY_FOCUS_MODE, 2);
                }
                ...
                if (this.mConfigFpsRange != null) {
                    builder.set((CaptureRequest.Key<Range<Integer>>) CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE, this.mConfigFpsRange);
                }
                ...
                if (builder.containCustomKey(PreviewParameter.KEY_FILTER_TYPE)) {
                    cameraRequestTag.mbVideoAicolorEnable = "on".equals(builder.get(PreviewParameter.KEY_VIDEO_RETENTION_OPEN));
                }
    ...
    protected CameraDeviceInfoInterface createCameraDeviceInfo(String str) {
        ...
        if (!Util.isSystemCamera() && "front_main".equals(str) && bool != null && bool.booleanValue()) {
            cameraDeviceInfoImpl.getConfigureParameterRangeMap().remove(ConfigureParameter.AI_NIGHT_VIDEO_MODE.getName());
        }
    ...
    public CameraRequestTag createRequestTag(String str, Object obj, Handler handler, String str2, PreviewParameter.Builder builder) {
        ...
        cameraRequestTagCreateRequestTag.mbAntibandingEnable = "on".equals(configureParameter.get(ConfigureParameter.ANTI_BANDING_ENABLE));
        cameraRequestTagCreateRequestTag.mbVideoWaterMarkEnable = "on".equals(configureParameter.get(ConfigureParameter.KEY_WATERMARK_VIDEO_ENABLE));
        cameraRequestTagCreateRequestTag.mbNormalVideoHFR = "video_120fps".equals(configureParameter.get(ConfigureParameter.VIDEO_FPS));
        cameraRequestTagCreateRequestTag.mbRearMirrorEnable = builder.get(PreviewParameter.KEY_REAR_MIRROR_ENABLE) != null && ((Boolean) builder.get(PreviewParameter.KEY_REAR_MIRROR_ENABLE)).booleanValue();
        cameraRequestTagCreateRequestTag.mbPictureVisualizationEnable = builder.get(PreviewParameter.KEY_PICTURE_VISUALIZATION_ENABLE) != null && "on".equals(builder.get(PreviewParameter.KEY_PICTURE_VISUALIZATION_ENABLE));
        ...
        if (CameraCharacteristicsHelper.isFrontCamera(cameraRequestTagCreateRequestTag.mCameraId) || cameraRequestTagCreateRequestTag.mbRearMirrorEnable) {
    ...
    private boolean isExplorerVideoBokehOpen(Parameter parameter, String str) {
        return ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_VIDEO_BLUR_V3_SUPPORT, false)).booleanValue() && "on".equals(parameter.get(ConfigureParameter.KEY_VIDEO_BLUR)) && "rear_main".equals(str);
    }
    ...
    public String getSurfaceUseCase(String str, boolean z) {
        ...
        return (zEquals && ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_APS_SUPPORT_VIDEO_SUPER_EIS, true)).booleanValue()) ? "eis_pro_case" : "on".equals(configureParameter.get(ConfigureParameter.AI_FOLLOW_ENABLE)) ? CameraConstant.UseCase.AI_FOLLOW_CASE : "recorder_video_case";
    }
    ...
    public boolean updateFpsRangeByVideoType(Parameter parameter, PreviewParameter.Builder builder, String str) {
        ...
        if (PlatformUtil.isMtkPlatform()) {
            ...
            boolean zIsFrontCamera = CameraCharacteristicsHelper.isFrontCamera(CameraCharacteristicsHelper.getCameraIdType(str).getCameraId());
            boolean zEquals = "super_stabilization_front".equals((String) parameter.get(ConfigureParameter.VIDEO_STABILIZATION_MODE));
            if ((num != null && 1 == num.intValue()) || ("1".equals(CameraConfigHelper.getConfigValue(DefaultUtill.KEY_FRONT_AE_FPS_RANGE_SUPPORT)) && zIsFrontCamera && !zEquals && "video_mode".equals(getModeName()))) {
    ...
    protected boolean isEndOfStreamNeeded(Parameter parameter, String str) {
        ...
        if ((bool2.booleanValue() && !"super_stabilization".equals(str2) && (VIDEO_4K_FRAME_WIDTH == this.mTargetVideoSize.getWidth() || "video_60fps".equals(str3) || ((range = this.mConfigFpsRange) != null && 60 == ((Integer) range.getUpper()).intValue()))) || !PlatformUtil.isQualcommPlatform()) {
            return false;
        }
    ...
    protected boolean isExplorerOpen(String str) {
        Parameter configureParameter = getConfigureParameter(str);
        if (!configureParameter.containCustomKey(ConfigureParameter.AI_NIGHT_VIDEO_MODE) || 1 != ((Integer) configureParameter.get(ConfigureParameter.AI_NIGHT_VIDEO_MODE)).intValue()) {
            return false;
        }
    ...
    protected boolean isPreviewProcessByAps(Parameter parameter, String str) {
        if (!Util.isSystemCamera() || ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_HIGH_FPS_VIDEO_PREVIEW_PROCESS_BY_APS, true)).booleanValue() || parameter == null || TextUtils.equals("on", (CharSequence) parameter.get(ConfigureParameter.KEY_WATERMARK_VIDEO_ENABLE))) {
            return true;
        }
        if (TextUtils.equals("super_stabilization", (CharSequence) parameter.get(ConfigureParameter.VIDEO_STABILIZATION_MODE))) {
            return ((Boolean) CameraConfigHelper.getConfigValue(CameraConfigBase.KEY_VIDEO_STABILIZATION_BY_APS, true)).booleanValue();
        }
        if (TextUtils.equals("super_stabilization_front", (CharSequence) parameter.get(ConfigureParameter.VIDEO_STABILIZATION_MODE))) {
