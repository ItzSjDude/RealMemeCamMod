...
public class FileUtilsWrapper {
    ...
    public static boolean copyFile(File srcFile, File destFile) {
        try {
            boolean result = FileUtils.copyFile(srcFile, destFile);
            return result;
        } catch (Throwable e) {
            Log.e(TAG, e.toString());
            return false;
    ...
    public static int setPermissions(File path, int mode, int uid, int gid) {
        try {
            int result = FileUtils.setPermissions(path, mode, uid, gid);
            return result;
        } catch (Throwable e) {
            Log.e(TAG, e.toString());
            return -1;
    ...
    public static int setPermissions(FileDescriptor fd, int mode, int uid, int gid) {
        try {
            int result = FileUtils.setPermissions(fd, mode, uid, gid);
            return result;
        } catch (Throwable e) {
            Log.e(TAG, e.toString());
            return -1;
